...
HX is a TypeScript application built on the [Pi SDK](https://github.com/badlogic/pi-mono). It embeds the Pi coding agent and extends it with the HX workflow engine, auto mode state machine, and project management primitives.
...
hx (CLI binary)
  └─ loader.ts          Sets PI_PACKAGE_DIR, HX env vars, dynamic-imports cli.ts
      └─ cli.ts         Wires SDK managers, loads extensions, starts InteractiveMode
          ...
          └─ src/resources/
              ├─ extensions/hx/    Core HX extension
              ├─ extensions/...     12 supporting extensions
              ├─ agents/            scout, researcher, worker
              ├─ AGENTS.md          Agent routing instructions
              └─ HX-WORKFLOW.md    Manual bootstrap protocol

hx headless              Headless mode — CI/cron orchestration via RPC child process
hx --mode mcp            MCP server mode — exposes tools over stdin/stdout

vscode-extension/         VS Code extension — chat participant (@hx), sidebar dashboard, RPC integration
```
...
`.hx/` is the sole source of truth. Auto mode reads it, writes it, and advances based on what it finds. No in-memory state survives across sessions. This enables crash recovery, multi-terminal steering, and session resumption.
...
`PI_PACKAGE_DIR` points to `pkg/` (not project root) to avoid Pi's theme resolution colliding with HX's `src/` directory. Contains only `piConfig` and theme assets.

### Always-Overwrite Sync

Bundled extensions and agents are synced to `~/.hx/agent/` on every launch, not just first run. This means `npm update -g` takes effect immediately.
...
|-----------|-----------------|
| **HX** | Core workflow engine — auto mode, state machine, commands, dashboard |
...
- **parser** — HX file parsing and frontmatter extraction
