...
If `.hx/SYSTEM.md` (project) or `~/.hx/agent/SYSTEM.md` (global) exists, its contents **completely replace** the default base prompt above. The tools list, guidelines, pi docs pointers — all gone. You own the entire base.
...
If `.hx/APPEND_SYSTEM.md` (project) or `~/.hx/agent/APPEND_SYSTEM.md` (global) exists, its contents are appended after the base prompt.
