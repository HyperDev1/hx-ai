...
- **No, you do not have to rely on `~/.hx`** if you embed pi with custom paths and storage
...
And inside `hx`, you import pi packages and create your own session, UI, storage, and resource loading behavior.

---

## 19.3 The Second Biggest Misconception: `~/.hx` Is a Default, Not a Requirement

Pi CLI defaults to `~/.hx/agent`, but embedded applications are not forced to use it.
...
- `~/.hx/agent`
- `~/Library/Application Support/HX`
- `%APPDATA%/HX`
...
This is the most natural fit for tools like `hx`.
...
- easiest way to remove dependency on `~/.hx`
...
|------|---------------------|
| Branded CLI like `hx` | `@mariozechner/pi-coding-agent` SDK |
...
## 19.6 The Recommended Path for a Branded CLI Like `hx`
...
const appRoot = path.join(os.homedir(), ".hx");
...
This is another place where people often assume they must rely on discovery from `~/.hx` or `.hx/`.
  ...
  extensionFactories: [
    (pi) => {
      pi.registerCommand("hello", {
        ...
        handler: async (_args, ctx) => ctx.ui.notify("Hello from HX", "info"),
      });
    },
  ],
});
...
const loader = new DefaultResourceLoader({
  ...
  systemPromptOverride: () => "You are HX, a specialized software delivery agent.",
});
```
...
### Recommendation
For a branded tool like `hx`, default to **bundled-app product** behavior.
...
```typescript
const authStorage = AuthStorage.create("/path/to/hx/auth.json");
```
...
```typescript
const modelRegistry = new ModelRegistry(authStorage, "/path/to/hx/models.json");
```
...
## 19.18 Building a Branded `hx` CLI: Recommended Shape
...
const resourceLoader = new DefaultResourceLoader({
  ...
  systemPromptOverride: () =>
    "You are HX, a branded software delivery agent. Prefer project-specific workflows and terminology.",
  additionalExtensionPaths: [
...
If your app silently loads from a user's `~/.hx`, you may get:
...
If your app is called `hx`, but state lives in `~/.hx`, users will notice.
...
- Good for productized tools like `hx`
...
> “I want users to download and run `hx`, and have it use pi internally without requiring a separate pi install or `~/.hx` setup.”
