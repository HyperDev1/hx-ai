...
HX's auto-mode is unreliable. Users experience:
...
2. **State corruption across branches** — `.hx/` planning artifacts (roadmaps, plans, decisions) are gitignored but branch-specific. Multiple branches sharing a single `.hx/` directory clobber each other's state. Users see wrong milestones marked complete, wrong roadmaps loaded, and auto-mode starting from the wrong phase.
...
| Zero `.hx/` state corruption | Manual worktrees created via `git worktree add` have correct `.hx/` state without any HX-specific initialization |
...
| Backwards compatibility | Existing projects with `hx/M001/S01` slice branches continue to work (read-only; new work uses new model) |
...
```
main
  └─ milestone/M001 (worktree at .hx/worktrees/M001/)
       ├─ hx/M001/S01 (slice branch — code + .hx/ artifacts)
...
### `.hx/` Tracking (Proposed — Coherent)

**Tracked (travels with branch):**
```
.hx/milestones/**/*.md    (except CONTINUE markers)
.hx/milestones/**/*.json  (META.json integration records)
.hx/PROJECT.md
.hx/DECISIONS.md
.hx/REQUIREMENTS.md
.hx/QUEUE.md
...
| `.hx/` state clobbering | Artifacts tracked in git. Each branch carries its own `.hx/`. `git worktree add` and `git checkout` give correct state. |
| Merge conflict complexity | No merges within a worktree. Only merge is milestone→main (squash). |
| Manual worktree initialization | Tracked artifacts are checked out with the branch. No HX-specific bootstrap needed. |
...
1. Update `.gitignore`:
   - Remove blanket `.hx/` ignore
   - Add explicit runtime-only ignores (see proposed list above)

2. Force-add existing planning artifacts on current branch:
   ```
   git add --force .hx/milestones/ .hx/PROJECT.md .hx/DECISIONS.md .hx/REQUIREMENTS.md .hx/QUEUE.md
   ```

3. Ensure runtime files are NOT tracked:
   ```
   git rm --cached -r .hx/runtime/ .hx/activity/ .hx/STATE.md .hx/metrics.json .hx/completed-units.json .hx/auto.lock
   ```
...
No conflict categorization. No runtime file stripping (runtime files are gitignored, not in the merge). No `.hx/` special handling.

If squash-merge conflicts (parallel milestone edge case): stop auto-mode with clear error, user resolves manually or HX dispatches a one-time resolution session.
...
2. Add new tests:
   ...
   - `.hx/` tracking: planning artifacts tracked, runtime files ignored
   - Manual worktree: `git worktree add` has correct `.hx/` state
...
1. State derivation (`deriveState()`) continues to read `hx/M001/S01` branch naming for legacy detection
2. On first run after upgrade:
   - Detect existing slice branches
   - Notify user: "HX no longer creates slice branches. Existing branches are preserved but new work commits directly to the milestone branch."
...
**Verification:** Open a project with existing `hx/M001/S01` branches. HX reads state correctly, new work commits on milestone branch without slice branches.
...
- Confirmed `.hx/milestones/` partially tracked on `main` despite `.gitignore`
...
| Need a replacement slice-boundary primitive | Accepted: conventional commit tags (`feat(M001/S01):`) + optional git tags (`hx/M001/S01-complete`) serve as boundaries. |
...
```
cd .hx/worktrees/M002
git fetch origin main
git rebase main
# Resolve any conflicts (code-only, never .hx/)
# Then squash-merge
```

This is standard git workflow. HX can automate the rebase step as a pre-merge check.
...
| Branch types | 4 (main, milestone/*, hx/*/*, worktree/*) |
...
- **M001 (Memory Database):** The SQLite database (`hx.db`) must remain gitignored. The M001/S02 importer layer rebuilds it from tracked markdown. This PRD's `.gitignore` update explicitly ignores `hx.db`.

- **PR #487:** Must be closed. The `resolveMainWorktreeRoot` approach (sharing `.hx/` across worktrees) contradicts tracked-artifact architecture.
...
2. **Should `worktrees/` move outside `.hx/`?** Having worktrees inside `.hx/` creates a nesting-doll pattern (worktree contains `.hx/` which is inside `.hx/worktrees/`). Relocating to `.hx-worktrees/` or `~/.hx/worktrees/<repo-hash>/` is cleaner but changes the filesystem layout. Recommendation: defer, address separately if it causes issues.

3. **Pre-flight rebase automation?** Before milestone→main squash-merge, should HX automatically `git rebase main`? Gemini recommends yes. Risk: rebase can fail with conflicts, adding a code path. Recommendation: implement as a doctor check ("milestone branch is behind main by N commits") with manual resolution, automate later if needed.
