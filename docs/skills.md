...
Skills are specialized instruction sets that HX loads when the task matches. They provide domain-specific guidance for the LLM — coding patterns, framework idioms, testing strategies, and tool usage.

Skills follow the open [Agent Skills standard](https://agentskills.io/) and are **not HX-specific** — they work with Claude Code, OpenAI Codex, Cursor, GitHub Copilot, Windsurf, and 40+ other agents.

## Skill Directories

HX reads skills from two locations, in priority order:
...
> **Migration from `~/.hx/agent/skills/`:** On first launch after upgrading, HX automatically copies skills from the legacy `~/.hx/agent/skills/` directory to `~/.agents/skills/`. The old directory is preserved for backward compatibility.
...
During `hx init`, HX detects the project's tech stack and recommends relevant skill packs. For brownfield projects, detection is automatic; for greenfield projects, the user picks a tech stack.

The curated catalog is maintained in `src/resources/extensions/hx/skill-catalog.ts`. Each entry maps a tech stack to a skills.sh repo and specific skill names.
...
The skill catalog lives in [`src/resources/extensions/hx/skill-catalog.ts`](../src/resources/extensions/hx/skill-catalog.ts). To add or update a pack:
...
The `skill_discovery` preference controls how HX finds skills during auto mode:
...
HX tracks skill performance across auto-mode sessions and surfaces health data to help you maintain skill quality.
...
View skill performance with `/hx skill-health`:

```
/hx skill-health              # overview table: name, uses, success%, tokens, trend, last used
/hx skill-health rust-core    # detailed view for one skill
/hx skill-health --stale 30   # skills unused for 30+ days
/hx skill-health --declining  # skills with falling success rates
```
...
When configured as a post-unit hook, HX can analyze whether the agent deviated from a skill's instructions during execution. If significant drift is detected (outdated API patterns, incorrect guidance), it writes proposed fixes to `.hx/skill-review-queue.md` for human review.
