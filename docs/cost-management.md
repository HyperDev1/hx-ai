...
HX tracks token usage and cost for every unit of work dispatched during auto mode. This data powers the dashboard, budget enforcement, and cost projections.
...
Data is stored in `.hx/metrics.json` and survives across sessions.

### Viewing Costs

**Dashboard:** `Ctrl+Alt+G` or `/hx status` shows real-time cost breakdown.
...
Once at least two slices have completed, HX projects the remaining cost:
...
- Check `/hx status` after a few slices to see per-slice cost averages
...
- Use `/hx visualize` → Metrics tab to see where your budget is going
