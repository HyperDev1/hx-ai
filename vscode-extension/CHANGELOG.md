...
- **Workflow controls** — sidebar buttons for Auto, Next, Quick Task, Capture, Status, and Fork that send `/hx` slash commands
...
- **Enhanced code lens** — Refactor, Find Bugs, and Generate Tests actions alongside Ask HX
...
- Full RPC client — spawns `hx --mode rpc`, JSON line framing, all RPC commands
- Sidebar dashboard — connection status, model info, thinking level, token usage, cost, quick actions
- Chat participant — `@hx` in VS Code Chat with streaming responses
...
- Slash command completion — auto-complete for `/hx` commands in editors
- Code lens — "Ask HX" above functions and classes in TS/JS/Python/Go/Rust
