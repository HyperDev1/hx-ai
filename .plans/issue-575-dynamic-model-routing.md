...
Users on capped plans (e.g., Claude Pro) exhaust weekly token limits in 15-20 hours of HX usage. Currently, HX uses a single model per phase (research/planning/execution/completion), configured statically in preferences. Simple tasks consume the same tokens as complex ones.
...
| File | Role |
|------|------|
| `src/resources/extensions/hx/auto.ts` | Dispatch logic, model switching (lines 1791-1879) |
| `src/resources/extensions/hx/preferences.ts` | Model resolution, `resolveModelWithFallbacksForUnit()` |
| `src/resources/extensions/hx/post-unit-hooks.ts` | Pre-dispatch hooks (model field defined but unused) |
| `src/resources/extensions/hx/types.ts` | Type definitions for hooks and model config |
| `src/resources/extensions/hx/metrics.ts` | Token tracking, aggregation, cost projection |
| `src/resources/extensions/hx/auto-prompts.ts` | Prompt builders per unit type |
...
**File:** `src/resources/extensions/hx/types.ts`
...
**File:** `src/resources/extensions/hx/preferences.ts`
...
**New file:** `src/resources/extensions/hx/complexity-classifier.ts`
...
**New file:** `src/resources/extensions/hx/model-router.ts`
...
**File:** `src/resources/extensions/hx/auto.ts`
...
**New file:** `src/resources/extensions/hx/tests/complexity-classifier.test.ts`
...
**New file:** `src/resources/extensions/hx/tests/model-router.test.ts`
...
**New file:** `src/resources/extensions/hx/tests/dynamic-routing-integration.test.ts`
...
**File:** `src/resources/extensions/hx/metrics.ts`
...
**File:** `src/resources/extensions/hx/auto-dashboard.ts`
...
**File:** `src/resources/extensions/hx/complexity-classifier.ts`
- Track success/failure per tier per unit-type pattern
- Store in `.hx/routing-history.json` (project-level)
...
Based on typical HX session patterns:
...
- New file: `src/resources/extensions/hx/model-cost-table.ts` — static cost table for known models, updatable via preferences
...
**Risk:** Cost data goes stale. Mitigate with a bundled cost table that gets updated with HX releases + user override capability.
...
- Post-unit prompt option: user can react with `/hx:rate-unit [over|under|ok]`
...
- Feedback stored alongside outcome data in `.hx/routing-history.json`
- Classifier weights feedback signals 2x vs. automatic success/failure detection
- Skill: `hx:rate-unit` — simple command that tags the last completed unit
...
| **3 — Adaptive Learning** | Outcome tracking, user feedback (`/hx:rate-unit`), adaptive thresholds | Decision 4 |
