...
Make the **left pane in Power User Mode** render the **real native HX/pi TUI** while staying attached to the **same authoritative main session** already used by:
...
At the same time, keep the **right pane unchanged** as a **separate PTY-backed HX session**.
...
The left pane is currently `web/components/hx/terminal.tsx`, which is **not** the native TUI. It is a browser-native summary / interaction surface backed by:
...
It renders bridge state, transcript summaries, tool activity, and an input box, but not the real pi/HX terminal UI.

### Right pane today

The right pane is `web/components/hx/shell-terminal.tsx`, backed by:
...
It launches a separate interactive `hx` process.
...
Refactor interactive-mode construction so it can accept a `Terminal` implementation from `/pi-tui` rather than always using `ProcessTerminal`.
...
Add a new terminal host for the left pane that implements the `/pi-tui` `Terminal` contract using browser transport instead of process stdin/stdout.
...
In `web/components/hx/dual-terminal.tsx`, replace the current left browser summary terminal with a new component that renders the real native TUI attached to the main session.

### Desired component behavior

- connect to the browser-backed main-session terminal transport
- render the actual native HX/pi TUI
...
- new browser component under `web/components/hx/`
...
Left pane visually and behaviorally matches native HX/pi TUI while remaining attached to the main session.
...
### Web left-pane UI
- `web/components/hx/dual-terminal.tsx`
- new bridge-native terminal component under `web/components/hx/`

### Existing right-pane UI to keep stable
- `web/components/hx/shell-terminal.tsx`
- `web/lib/pty-manager.ts`
...
- `web/components/hx/command-surface.tsx`
