...
> **Phase 2 (Global Setup refactor)**: Deferred — `/hx setup` provides routing to existing commands for now

## Problem Statement

HX currently has two disconnected onboarding paths:

1. **App onboarding** (`onboarding.ts`) — pre-TUI, only handles LLM/tool auth. Runs once ever, no project awareness.
2. **Project bootstrap** (`showSmartEntry()`) — silently creates `.hx/` and drops you into the discuss prompt with zero explanation.

Neither detects v1 `.planning/` directories, explains what HX is, offers project-level configuration, or helps returning users entering a new folder.

## Design Decisions

### Q1: Should bare `/hx` show a menu or auto-start?

**Answer: Contextual behavior based on detection.**

| Detected State | Behavior |
|---|---|
| No `.hx/`, no `.planning/` | **Init Wizard** (new project onboarding) |
| No `.hx/`, has `.planning/` | **Migration offer** → `/hx migrate` or skip |
| Has `.hx/`, no milestones | Current flow (discuss prompt) |
| Has `.hx/`, has milestones | Current flow (smart entry / auto resume) |
| First-ever HX launch (no `~/.hx/`) | **Global setup** first, then project init |

### Q2: Should there be an onboarding wizard?

**Answer: Yes — two-tier wizard.**

- **Global wizard** (`/hx setup`) — runs once per machine. Handles: LLM auth (absorb current `onboarding.ts`), global preferences (default model, mode, timeout defaults), tool keys, remote questions.
- **Project wizard** (`/hx init`) — runs once per project folder. Handles: project-type detection, preferences template, git init, `.gitignore`, optional CONTEXT.md seeding.

Both wizards should be:
- Skippable at every step
- Re-runnable (`/hx setup` and `/hx init` work any time)
...
### New File: `src/resources/extensions/hx/init-wizard.ts`

Project-level init wizard. Responsible for the interactive experience when entering a new folder.

### New File: `src/resources/extensions/hx/detection.ts`

Pure detection functions. No UI, no side effects.

### Modified: `src/resources/extensions/hx/guided-flow.ts`

`showSmartEntry()` gains a detection preamble before the current logic.

### Modified: `src/resources/extensions/hx/commands.ts`

New subcommands: `/hx init`, `/hx setup`. Existing `/hx migrate` stays.

### Modified: `src/onboarding.ts`

Refactored to be callable from both pre-TUI boot and `/hx setup`.
...
interface ProjectDetection {
  ...
  /** v2 details (if state === 'v2-hx' or 'v2-hx-empty') */
  v2?: {
...
Returns `true` if `~/.hx/` doesn't exist or has no `PREFERENCES.md`.

---

## Phase 2: Global Setup Wizard (`/hx setup`)
...
### Task 2.2: `/hx setup` command handler

```
/hx setup          → full wizard (all steps)
/hx setup llm      → just LLM auth
/hx setup search   → just web search
/hx setup remote   → just remote questions
/hx setup keys     → just tool keys
/hx setup prefs    → just global preferences
```
...
│  ✓ Remote questions: Discord #hx-bot        │
...
## Phase 3: Project Init Wizard (`/hx init`)
...
| **`git.commit_docs`** | Whether `.hx/` plans get committed to git. Team projects usually want `true`, throwaway prototypes want `false`. Affects the very first commit. | `true` |
...
#### Tier 3: Don't ask at init (defer to `/hx prefs project`)
...
Step 1: Detection scan (automatic, instant, no prompt)
   ...
   ├─ .hx/ already exists? → Re-init safety (Task 3.3)
...
Step 5: Verification Commands (auto-populated from detection)
   "How should HX verify code changes?"
...
Step 6: Git Preferences
   "Git settings for this project:"
     Commit .hx/ plans to git?  [Y/n]         ← default yes
...
Step 7: Project Instructions (optional, skippable)
   "Any rules HX should follow for this project?"
...
Step 9: Bootstrap .hx/ structure
   - Creates .hx/milestones/
   - Creates .hx/PREFERENCES.md (from wizard answers)
   - Creates .gitignore entries
   - Seeds CONTEXT.md with detected project signals
   - Commits "chore: init hx" (if commit_docs enabled)
...
```
┌ HX — Legacy Project Detected ────────────────┐
│                                               │
│  Found .planning/ directory (HX v1 format)   │
│  3 phases, 12 tasks detected                  │
│                                               │
│  > Migrate to HX v2    (recommended)         │
│    Start fresh                                │
│    Cancel                                     │
│                                               │
└───────────────────────────────────────────────┘
...
If `.hx/` already exists when `/hx init` is run:
...
// No .hx/ — run project init wizard
if (detection.state === 'none') {
  await showProjectInit(ctx, pi, basePath, detection);
  return;
}

// Existing .hx/ — current logic continues unchanged
```

### Task 4.2: Preserve zero-friction for returning users

The detection + init wizard only triggers when `.hx/` doesn't exist.
Once `.hx/` exists, the flow is identical to today — no regressions.
...
- `isFirstEverLaunch()` with/without `~/.hx/`

### Task 5.2: Init wizard integration tests

- New folder → wizard triggers → `.hx/` created correctly
- v1 folder → migration offer shown
- Existing `.hx/` → wizard skipped, normal flow
- Re-run `/hx init` on existing project → safe behavior

### Task 5.3: Global setup tests

- `/hx setup` from command handler
- Individual sub-steps (`/hx setup llm`, etc.)
...
| File | Action | Purpose |
|------|--------|---------|
| `src/resources/extensions/hx/detection.ts` | **NEW** | Pure detection functions |
| `src/resources/extensions/hx/init-wizard.ts` | **NEW** | Project init wizard UI |
| `src/resources/extensions/hx/global-setup.ts` | **NEW** | Global setup wizard (refactored from onboarding.ts) |
| `src/onboarding.ts` | **MODIFY** | Delegate to global-setup.ts, keep boot integration |
| `src/resources/extensions/hx/guided-flow.ts` | **MODIFY** | Add detection preamble to showSmartEntry() |
| `src/resources/extensions/hx/commands.ts` | **MODIFY** | Add `/hx init` and `/hx setup` subcommands |
| Tests (TBD paths) | **NEW** | Detection, init, setup tests |

## Open Questions for Discussion

1. **Should `/hx init` auto-transition to the discuss prompt?** Or end with "Run /hx to start"? Auto-transition is more fluid but might feel jarring after a wizard.
...
3. **Should `/hx setup` be accessible outside the TUI?** e.g. `hx setup` from the shell before launching. Currently `onboarding.ts` handles this but it's limited.

4. **Migration: should we auto-detect `.planning/` in parent directories?** Some users might run HX from a subdirectory while `.planning/` is at the repo root.
