# HX Startup Performance Analysis & Optimization Plan

## Measured Baseline (macOS, Node v25.6.1)

### `hx --version` (simplest possible path): **2.2 seconds**
...
| **/pi-coding-agent barrel import** | **~970ms** | THE BOTTLENECK |
...
| Phase | Time | Notes |
|-------|------|-------|
| /pi-coding-agent import | ~750ms | (cached from loader measurement) |
...
### Inside /pi-coding-agent (barrel import breakdown)
...
### 2. Barrel import of /pi-coding-agent (~1s)
...
`cpSync` with `force: true` copies all bundled resources to `~/.hx/agent/` on every startup, even when nothing changed.
...
**Impact**: `hx --version` goes from 2.2s → ~0.2s

#### 1B. Skip initResources when unchanged
Compare `managed-resources.json` version against current `HX_VERSION`. If they match, skip the `cpSync` entirely.
