...
Four native Rust optimizations to eliminate hot-path bottlenecks in HX's dispatch cycle.
...
### Problem
`state.ts:134-176` — When `nativeBatchParseHxFiles()` returns parsed files, the JS
...
Add a new field `rawContent: String` to `ParsedHxFile` that contains the complete
...
### Implementation
- **Rust** (`hx_parser.rs`): Add `raw_content` field to `ParsedHxFile`, populate with
...
### Implementation
- **Rust** (`hx_parser.rs`): Add `parse_jsonl_tail(path, max_entries?)` function that:
...
### Solution
Add a Rust function that walks the entire `.hx/` tree once and returns a flat
...
### Implementation
- **Rust** (`hx_parser.rs`): The `batchParseHxFiles` already walks the tree.
  Add `scan_hx_tree(directory)` that returns `Vec<{ path, isDir, name }>` for
...
### Implementation
- **Rust** (`hx_parser.rs`): Add `parse_plan_file(content)` and `parse_summary_file(content)`.
...
### Rust
- `native/crates/engine/src/hx_parser.rs` — new functions + rawContent field

### TypeScript
- `src/resources/extensions/hx/native-parser-bridge.ts` — new bridge functions
- `src/resources/extensions/hx/state.ts` — simplified batch cache
- `src/resources/extensions/hx/paths.ts` — native tree cache
- `src/resources/extensions/hx/session-forensics.ts` — native JSONL
- `src/resources/extensions/hx/files.ts` — native plan/summary parsers
