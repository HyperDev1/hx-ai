...
HX has solid API key infrastructure (AuthStorage, OAuth flows, rate-limit backoff, multi-key rotation) but lacks a user-facing CLI for day-to-day key management. Users currently must either:
- Run the full onboarding wizard to add keys
- Manually edit `~/.hx/agent/auth.json`
- Use the limited `/hx setup keys` flow (only covers 5 tool keys, no LLM keys)

There's no way to list, test, remove, or inspect key health from the CLI.

## Scope

Build `/hx keys` — a comprehensive API key management command with subcommands:

```
/hx keys                    → Show key status dashboard
/hx keys list               → List all configured keys with status
/hx keys add <provider>     → Add/replace a key for a provider
/hx keys remove <provider>  → Remove a key for a provider
/hx keys test [provider]    → Validate key(s) by making a lightweight API call
/hx keys rotate <provider>  → Remove old key and prompt for new one
/hx keys doctor             → Health check all keys (expired OAuth, empty keys, backoff state)
```
...
| File | Purpose |
|------|---------|
| `src/resources/extensions/hx/key-manager.ts` | Core key manager logic (list, add, remove, test, rotate, doctor) |
| `src/resources/extensions/hx/tests/key-manager.test.ts` | Unit tests |

### Modified Files

| File | Change |
|------|--------|
| `src/resources/extensions/hx/commands.ts` | Add `/hx keys` subcommand routing + completions |

### No changes to core packages

All work stays in the HX extension layer. We use `AuthStorage` as-is — no modifications to `pi-coding-agent` or `pi-ai`.

---

## Phase 1: Key Status Dashboard (`/hx keys` and `/hx keys list`)
...
## Phase 2: Add Key (`/hx keys add <provider>`)
...
## Phase 3: Remove Key (`/hx keys remove <provider>`)
...
## Phase 4: Test Key (`/hx keys test [provider]`)
...
## Phase 5: Rotate Key (`/hx keys rotate <provider>`)
...
4. Optionally test the new key before saving (`/hx keys test` logic)
...
## Phase 6: Key Doctor (`/hx keys doctor`)
...
API Key Health Check
  ...
  ✗ groq: empty key stored (from skipped setup) — run /hx keys add groq
...
Add to the `/hx` subcommand router:
```typescript
