...
Replaces: `git diff --name-status main...branch -- .hx/` (worktree-manager.ts, 3 calls)
...
Replaces: `git diff main...branch -- .hx/` and `-- . :(exclude).hx/` (worktree-manager.ts, 2 calls)
...
Replaces: `git branch --list milestone/*`, `git branch --list hx/*` (doctor.ts, commands.ts, 3 calls)
Returns: Branch names matching pattern

### 1.10 — `git_branch_list_merged(repo_path, target, pattern?) -> Vec<String>`
Replaces: `git branch --merged main --list hx/*` (commands.ts, 1 call)
...
Replaces: `git for-each-ref refs/hx/snapshots/ --format=%(refname)` (commands.ts, 1 call)
...
- `git add -A .hx .gitignore && git commit` → use `nativeAddPaths()` + `nativeCommit()`
