...
describe('sanitizeChannelName', () => {
  it('converts basic path to hx-prefixed name', () => {
    assert.equal(sanitizeChannelName('/home/user/my-project'), 'hx-my-project');
  });

  it('converts path with special characters to hyphens', () => {
    assert.equal(sanitizeChannelName('/home/user/My_Cool.Project!v2'), 'hx-my-cool-project-v2');
  });
  ...
  it('truncates very long names to 100 chars', () => {
    ...
    assert.ok(result.startsWith('hx-'));
  });

  it('cleans leading/trailing dots and underscores', () => {
    assert.equal(sanitizeChannelName('/home/...___project___...'), 'hx-project');
  });

  it('returns hx-unnamed for empty basename', () => {
    assert.equal(sanitizeChannelName(''), 'hx-unnamed');
    assert.equal(sanitizeChannelName('/'), 'hx-unnamed');
  });

  it('returns hx-unnamed for basename with only special chars', () => {
    assert.equal(sanitizeChannelName('/home/!!!'), 'hx-unnamed');
  });

  it('collapses consecutive hyphens', () => {
    assert.equal(sanitizeChannelName('/home/a---b---c'), 'hx-a-b-c');
  });

  it('handles Windows-style backslash paths', () => {
    assert.equal(sanitizeChannelName('C:\\Users\\lex\\my-project'), 'hx-my-project');
  });

  it('handles name at exact prefix + 96 chars = 100 char limit', () => {
    // hx- is 4 chars, so a 96-char basename should produce exactly 100
    const name96 = 'a'.repeat(96);
    const result = sanitizeChannelName(`/home/${name96}`);
    assert.equal(result.length, 100);
    assert.equal(result, `hx-${'a'.repeat(96)}`);
  });
...
describe('ChannelManager', () => {
  ...
  it('resolveCategory creates category when not found', async () => {
    ...
    assert.equal(cat.name, 'HX Projects');
  ...
  it('createProjectChannel creates text channel under category', async () => {
    ...
    assert.equal(channel.name, 'hx-my-project');
...
describe('buildCommands', () => {
  it('returns array with correct command names', () => {
    ...
    assert.ok(names.includes('hx-status'), 'should include hx-status');
    assert.ok(names.includes('hx-start'), 'should include hx-start');
    assert.ok(names.includes('hx-stop'), 'should include hx-stop');
    assert.ok(names.includes('hx-verbose'), 'should include hx-verbose');
  });
...
describe('command dispatch', () => {
  ...
  it('hx-status with no sessions produces empty message', () => {
    // Tests the formatSessionStatus path that /hx-status calls
  ...
  it('unknown command name is not in buildCommands list', () => {
    const commands = buildCommands();
    const names = commands.map((c) => c.name);
    assert.ok(!names.includes('hx-unknown'), 'unknown should not be in command list');
  });
...
// ---------- /hx-start and /hx-stop logic paths ----------

describe('/hx-start and /hx-stop logic', () => {
  // These test the observable logic paths exercised by the handlers.
  // Since handleHxStart/handleHxStop are private, we test the data layer
  // they depend on — project scanning, session listing, and edge cases.

  it('/hx-start: scanForProjects returning 0 projects', async () => {
  ...
  it('/hx-stop: getAllSessions returns empty when no sessions active', async () => {
  ...
  it('/hx-stop: filters to active sessions only', () => {
    // Simulate the filter logic used in handleHxStop
  ...
  it('/hx-start: >25 projects are truncated for select menu', () => {
