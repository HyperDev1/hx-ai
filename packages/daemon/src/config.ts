...
function defaults(): DaemonConfig {
  return {
    ...
    projects: { scan_roots: [] },
    log: {
      file: resolve(homedir(), '.hx', 'daemon.log'),
      level: 'info',
      max_size_mb: 50,
    },
  };
 ...
 * Priority: explicit CLI arg → HX_DAEMON_CONFIG env → ~/.hx/daemon.yaml
 */
export function resolveConfigPath(cliPath?: string): string {
  if (cliPath) return expandTilde(cliPath);
  const envPath = process.env['HX_DAEMON_CONFIG'];
  if (envPath) return expandTilde(envPath);
  return resolve(homedir(), '.hx', 'daemon.yaml');
}
