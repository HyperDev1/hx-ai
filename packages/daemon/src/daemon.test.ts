...
describe('resolveConfigPath', () => {
  ...
  it('falls back to HX_DAEMON_CONFIG env var', () => {
    const prev = process.env['HX_DAEMON_CONFIG'];
    try {
      process.env['HX_DAEMON_CONFIG'] = '/env/path.yaml';
      const p = resolveConfigPath();
      assert.equal(p, '/env/path.yaml');
    } finally {
      if (prev === undefined) delete process.env['HX_DAEMON_CONFIG'];
      else process.env['HX_DAEMON_CONFIG'] = prev;
    }
  });

  it('defaults to ~/.hx/daemon.yaml', () => {
    const prev = process.env['HX_DAEMON_CONFIG'];
    try {
      delete process.env['HX_DAEMON_CONFIG'];
      const p = resolveConfigPath();
      assert.equal(p, join(homedir(), '.hx', 'daemon.yaml'));
    } finally {
      if (prev !== undefined) process.env['HX_DAEMON_CONFIG'] = prev;
    }
  });
});
...
describe('CLI integration', () => {
  it('--help prints usage and exits 0', () => {
    ...
    assert.ok(result.includes('Usage: hx-daemon'));
