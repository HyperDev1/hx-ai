...
function basePlistOpts(overrides?: Partial<PlistOptions>): PlistOptions {
  return {
    ...
    scriptPath: '/usr/local/lib/hx-daemon/dist/cli.js',
    configPath: join(homedir(), '.hx', 'daemon.yaml'),
    ...overrides,
  };
}
...
describe('generatePlist', () => {
  ...
  it('includes label com.hx.daemon', () => {
    const xml = generatePlist(basePlistOpts());
    assert.ok(xml.includes('<string>com.hx.daemon</string>'));
  });
...
describe('getPlistPath', () => {
  it('returns ~/Library/LaunchAgents/com.hx.daemon.plist', () => {
    const expected = join(homedir(), 'Library', 'LaunchAgents', 'com.hx.daemon.plist');
...
describe('status', () => {
  ...
  it('returns not-registered when launchctl list fails', () => {
    const mockRun: RunCommandFn = (_cmd: string) => {
      throw new Error('Could not find service "com.hx.daemon" in domain for port');
    };
  ...
  it('parses JSON-style dict output (newer macOS)', () => {
    const mockRun: RunCommandFn = (_cmd: string) => {
      return `{
\t"StandardOutPath" = "/Users/me/.hx/daemon-stdout.log";
\t"LimitLoadToSessionType" = "Aqua";
\t"StandardErrorPath" = "/Users/me/.hx/daemon-stderr.log";
\t"Label" = "com.hx.daemon";
\t"OnDemand" = true;
\t"LastExitStatus" = 0;
\t"PID" = 23802;
