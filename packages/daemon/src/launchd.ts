...
/** Generate valid launchd plist XML for the HX daemon. */
export function generatePlist(opts: PlistOptions): string {
  const home = homedir();
  const workDir = opts.workingDirectory ?? home;
  const stdoutPath = opts.stdoutPath ?? resolve(home, '.hx', 'daemon-stdout.log');
  const stderrPath = opts.stderrPath ?? resolve(home, '.hx', 'daemon-stderr.log');
