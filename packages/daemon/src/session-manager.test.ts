 ...
 * and cleanup without spawning real HX processes.
 */
...
describe('SessionManager', () => {
  ...
  it('sends custom command when provided', async () => {
    ...
    await manager.startSession({ projectDir: '/tmp/custom-cmd', command: '/hx quick fix-typo' });
    const client = manager.lastClient!;

    assert.ok(client.prompted.includes('/hx quick fix-typo'));
    assert.ok(!client.prompted.includes('/hx auto'));
  });
