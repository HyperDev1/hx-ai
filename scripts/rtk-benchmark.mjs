...
function getManagedRtkPath() {
  return join(homedir(), '.hx', 'agent', 'bin', process.platform === 'win32' ? 'rtk.exe' : 'rtk')
}
...
function main() {
  ...
  const binaryPath = process.env.HX_RTK_PATH || getManagedRtkPath()

  if (!binaryPath) {
    throw new Error('RTK binary path not resolved')
  }

  const workspace = mkdtempSync(join(tmpdir(), 'hx-rtk-benchmark-'))
