...
describe("shutdown-gate", () => {
  afterEach(() => {
    ...
    cancelShutdown();
    delete process.env.HX_WEB_DAEMON_MODE;
  });
  ...
  describe("daemon mode (HX_WEB_DAEMON_MODE=1)", () => {
    beforeEach(() => {
      process.env.HX_WEB_DAEMON_MODE = "1";
    });
  ...
  describe("daemon mode is not activated by other values", () => {
    test("HX_WEB_DAEMON_MODE=0 does not enable daemon mode", () => {
      process.env.HX_WEB_DAEMON_MODE = "0";
      assert.equal(isDaemonMode(), false);
      scheduleShutdown();
      assert.equal(isShutdownPending(), true);
    });

    test("HX_WEB_DAEMON_MODE=true does not enable daemon mode", () => {
      process.env.HX_WEB_DAEMON_MODE = "true";
      assert.equal(isDaemonMode(), false);
      scheduleShutdown();
      assert.equal(isShutdownPending(), true);
    });

    test("unset HX_WEB_DAEMON_MODE does not enable daemon mode", () => {
      delete process.env.HX_WEB_DAEMON_MODE;
      assert.equal(isDaemonMode(), false);
      scheduleShutdown();
      assert.equal(isShutdownPending(), true);
