...
function bootSeedLines(boot: WorkspaceBootPayload): WorkspaceTerminalLine[] {
  const lines = [
    createTerminalLine("system", `HX web workspace attached to ${boot.project.cwd}`),
...
function createInitialState(): WorkspaceStoreState {
  return {
    ...
    terminalLines: [createTerminalLine("system", "Preparing the live HX workspace…")],
    ...
    switch (outcome.kind) {
      ...
      case "view-navigate": {
        ...
        window.dispatchEvent(
          new CustomEvent("hx:navigate-view", { detail: { view: outcome.view } }),
        )
        return outcome
      }
    }
