...
function getHxRoot(projectCwd: string): string {
  return join(projectCwd, ".hx");
}

function getRootForMode(mode: RootMode, projectCwd: string): string {
  return mode === "project" ? projectCwd : getHxRoot(projectCwd);
}
...
export async function GET(request: Request): Promise<Response> {
  ...
  const rootParam = (searchParams.get("root") ?? "hx") as RootMode;

  if (rootParam !== "hx" && rootParam !== "project") {
    return Response.json(
      { error: `Invalid root: must be "hx" or "project"` },
      { status: 400 },
    );
  }
  ...
  if (!resolvedPath) {
    const label = rootParam === "project" ? "project root" : ".hx/";
    return Response.json(
...
export async function POST(request: Request): Promise<Response> {
  ...
  const { path: pathParam, content, root: rootParam = "hx" } = body as {
    path?: string;
    content?: unknown;
    root?: string;
  };
...
export async function PATCH(request: Request): Promise<Response> {
  ...
  const { from, to, root: rootParam = "hx" } = body as {
    from?: string;
    to?: string;
    root?: string;
  };
...
export async function PUT(request: Request): Promise<Response> {
  ...
  const { path: pathParam, type = "file", root: rootParam = "hx" } = body as {
    path?: string;
    type?: "file" | "directory";
    root?: string;
  };
