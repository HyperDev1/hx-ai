...
/* ─── HX Action Definitions ─── */

/**
 * Defines every /hx subcommand available in the chat input bar.
...
const HX_ACTIONS: HXActionDef[] = [
  ...
  { label: "Auto",      command: "/hx auto",      icon: Zap,               description: "Run all queued units continuously",                         category: "workflow" },
  // ── Overflow: Workflow ──
  { label: "Stop",      command: "/hx stop",      icon: Square,            description: "Stop auto-mode gracefully",                                  category: "workflow" },
  { label: "Pause",     command: "/hx pause",     icon: Pause,             description: "Pause auto-mode (preserves state)",                          category: "workflow" },
  // ── Overflow: Visibility ──
  { label: "Status",    command: "/hx status",    icon: BarChart3,         description: "Show progress dashboard",                                    category: "visibility" },
  { label: "Visualize", command: "/hx visualize", icon: LayoutGrid,        description: "Interactive TUI (progress, deps, metrics, timeline)",        category: "visibility" },
  { label: "Queue",     command: "/hx queue",     icon: ListOrdered,       description: "Show queued/dispatched units and execution order",            category: "visibility" },
  { label: "History",   command: "/hx history",   icon: History,           description: "View execution history with cost/phase/model details",        category: "visibility" },
  // ── Overflow: Course correction ──
  { label: "Steer",     command: "/hx steer",     icon: Compass,           description: "Apply user override to active work",                         category: "correction" },
  { label: "Capture",   command: "/hx capture",   icon: PenLine,           description: "Quick-capture a thought to CAPTURES.md",                     category: "correction" },
  { label: "Triage",    command: "/hx triage",    icon: Inbox,             description: "Classify and route pending captures",                        category: "correction",  disabledDuringAuto: true },
  { label: "Skip",      command: "/hx skip",      icon: SkipForward,       description: "Prevent a unit from auto-mode dispatch",                     category: "correction" },
  { label: "Undo",      command: "/hx undo",      icon: Undo2,             description: "Revert last completed unit",                                 category: "correction" },
  // ── Overflow: Knowledge ──
  { label: "Knowledge", command: "/hx knowledge", icon: BookOpen,          description: "Add rule, pattern, or lesson to KNOWLEDGE.md",               category: "knowledge" },
  // ── Overflow: Configuration ──
  { label: "Mode",      command: "/hx mode",      icon: SlidersHorizontal, description: "Set workflow mode (solo/team)",                               category: "config" },
  { label: "Prefs",     command: "/hx prefs",     icon: Settings,          description: "Manage preferences (global/project)",                        category: "config" },
  // ── Overflow: Maintenance ──
  { label: "Doctor",    command: "/hx doctor",    icon: Stethoscope,       description: "Diagnose and repair .hx/ state",                            category: "maintenance" },
  { label: "Export",    command: "/hx export",    icon: FileOutput,        description: "Export milestone/slice results (JSON or Markdown)",           category: "maintenance" },
  { label: "Cleanup",   command: "/hx cleanup",   icon: Trash2,            description: "Remove merged branches or snapshots",                        category: "maintenance" },
  { label: "Remote",    command: "/hx remote",    icon: Globe,             description: "Control remote auto-mode (Slack/Discord)",                    category: "maintenance" },
]
 ...
 *
 * All /hx commands dispatch through the main bridge session.
 ...
 *   - sessionStorage key "hx-active-view:<cwd>" equals "chat" when this view is active.
...
function ChatModeHeader({ onPrimaryAction, onSecondaryAction }: ChatModeHeaderProps) {
  ...
  // Derive a short HX state badge label
  const stateBadge = (() => {
...
/**
 * Renders a HX arrow-key select prompt as a native clickable list.
 *
...
/**
 * Renders a HX text prompt as a native labeled input field.
 *
...
/**
 * Renders a HX password/API-key prompt as a native masked input field.
 *
 ...
 * - Overflow menu (⋯) contains all remaining /hx subcommands grouped by category
    ...
    <div className="flex flex-1 flex-col items-center justify-center text-center py-16">
      ...
      <div className="mt-3 space-y-1">
        ...
        {showSpinner ? (
          ...
          <p className="max-w-xs text-xs text-muted-foreground">
            Connected — waiting for HX output…
          </p>
        )}
      </div>
    </div>
