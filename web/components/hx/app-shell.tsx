...
import { Sidebar, MilestoneExplorer, CollapsedMilestoneSidebar } from "@/components/hx/sidebar"
import { ShellTerminal } from "@/components/hx/shell-terminal"
import { Dashboard } from "@/components/hx/dashboard"
import { Roadmap } from "@/components/hx/roadmap"
import { FilesView } from "@/components/hx/files-view"
import { ActivityView } from "@/components/hx/activity-view"
import { VisualizerView } from "@/components/hx/visualizer-view"
import { StatusBar } from "@/components/hx/status-bar"
import { DualTerminal } from "@/components/hx/dual-terminal"
import { FocusedPanel } from "@/components/hx/focused-panel"
import { OnboardingGate } from "@/components/hx/onboarding-gate"
import { CommandSurface } from "@/components/hx/command-surface"
...
import { ProjectsPanel, ProjectSelectionGate } from "@/components/hx/projects-view"
import { UpdateBanner } from "@/components/hx/update-banner"
...
function viewStorageKey(projectCwd: string): string {
  return `hx-active-view:${projectCwd}`
}
...
function WorkspaceChrome() {
  ...
  useEffect(() => {
    const restoreTimer = window.setTimeout(() => {
      try {
        const stored = localStorage.getItem("hx-sidebar-collapsed")
  ...
  useEffect(() => {
    try {
      localStorage.setItem("hx-sidebar-collapsed", String(sidebarCollapsed))
    } catch {
      // localStorage may be unavailable
    }
  }, [sidebarCollapsed])
  ...
  useEffect(() => {
    ...
    const base = projectLabel ? `HX - ${projectLabel}` : "HX"
  ...
  useEffect(() => {
    ...
    window.addEventListener("hx:open-file", handler)
    return () => window.removeEventListener("hx:open-file", handler)
  }, [])

  // Listen for cross-component view navigation events (e.g. /hx visualize dispatch)
  useEffect(() => {
    ...
    window.addEventListener("hx:navigate-view", handler as EventListener)
    return () => window.removeEventListener("hx:navigate-view", handler as EventListener)
  }, [handleViewChange])

  // Listen for projects panel toggle (sidebar icon, or programmatic)
  useEffect(() => {
    const handler = () => setProjectsPanelOpen(true)
    window.addEventListener("hx:open-projects", handler)
    return () => window.removeEventListener("hx:open-projects", handler)
  }, [])
  ...
  const isWelcomeState =
    ...
    detection != null &&
    detection.kind !== "active-hx" &&
    detection.kind !== "empty-hx"
      ...
      <div className="flex h-dvh flex-col items-center justify-center gap-6 bg-background p-8 text-center">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="max-w-sm text-sm text-muted-foreground">
            ...
            <code className="rounded bg-muted px-1 py-0.5 font-mono text-xs">hx --web</code>.
          </p>
        </div>
      </div>
    )
