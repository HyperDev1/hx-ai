 ...
 * into HX's auth storage. Runs when HX has no LLM providers configured,
...
import type { AuthStorage, AuthCredential } from '@hyperlab/hx-coding-agent'
...
/**
 * Migrate provider credentials from Pi's auth.json into HX's AuthStorage.
 *
 * Only runs when HX has no LLM provider configured and Pi's auth.json exists.
 * Copies any credentials HX doesn't already have. Returns true if an LLM
...
export function migratePiCredentials(authStorage: AuthStorage): boolean {
  try {
    ...
    for (const [providerId, credential] of Object.entries(piData)) {
      ...
      process.stderr.write(`[hx] Migrated ${isLlm ? 'LLM provider' : 'credential'}: ${providerId} (from Pi)\n`)
    }
