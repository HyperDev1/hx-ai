    ...
    if not ok:
        if "externally-managed" in detail.lower():
            emit(
                ...
                "Create a venv first: python3 -m venv ~/.hx/voice-venv && "
                "~/.hx/voice-venv/bin/pip install sounddevice requests",
            )
...
def run_groq():
    ...
    if not api_key:
        emit("ERROR", "GROQ_API_KEY not set. Run 'hx config' to set up, or get a free key at https://console.groq.com")
        sys.exit(1)
