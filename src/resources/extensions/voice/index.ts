...
import type { AssistantMessage } from "@hyperlab/hx-ai";
import { isKeyRelease, Key, matchesKey, truncateToWidth, visibleWidth } from "@hyperlab/hx-tui";
...
function ensureLinuxReady(ctx: ExtensionContext): boolean {
	...
	if (!process.env.GROQ_API_KEY) {
		ctx.ui.notify("Voice: GROQ_API_KEY not set — run 'hx config' to configure", "error");
		return false;
	}
