...
function makeDeps(
  ...
  const deps: WorktreeResolverDeps = {
    ...
    createAutoWorktree: (_basePath: string, milestoneId: string) =>
      `/project/.hx/worktrees/${milestoneId}`,
    ...
    resolveMilestoneFile: (_basePath: string, milestoneId: string) =>
      `/project/.hx/milestones/${milestoneId}/${milestoneId}-ROADMAP.md`,
...
/** Read all journal entries from a temp .hx/journal directory. */
function readJournalEntries(basePath: string): JournalEntry[] {
  const journalDir = join(basePath, ".hx", "journal");
  try {
