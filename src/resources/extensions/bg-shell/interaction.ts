...
export async function queryShellEnv(
	...
	const sentinel = `__HX_ENV_${randomUUID().slice(0, 8)}__`;
...
export async function runOnSession(
	...
	const startMarker = `__HX_SENTINEL_${sentinel}_START__`;
	const endMarker = `__HX_SENTINEL_${sentinel}_END__`;
	const exitVar = `__HX_EXIT_${sentinel}__`;
