/**
 * GitHub Sync extension for HX.
 *
 * Opt-in extension that syncs HX lifecycle events to GitHub:
...
async function showStatus(ctx: import("@hyperlab/hx-coding-agent").ExtensionCommandContext) {
  if (!ghIsAvailable()) {
...
async function runBootstrap(ctx: import("@hyperlab/hx-coding-agent").ExtensionCommandContext) {
  if (!ghIsAvailable()) {
    ctx.ui.notify("GitHub sync: `gh` CLI not installed or not authenticated.", "warning");
    return;
  }
