...
import { buildTaskCommitMessage } from "../../hx/git-service.ts";
...
describe("commit linking", () => {
  it("appends Resolves #N when issueNumber is set", () => {
    ...
    assert.ok(msg.includes("HX-Task: S01/T02"), "HX-Task trailer present");
  });
  ...
  it("includes both key files and Resolves #N", () => {
    ...
    assert.ok(msg.includes("HX-Task: S01/T02"), "HX-Task trailer present");
    // HX-Task should come after key files but before Resolves
    const keyFilesIdx = msg.indexOf("- src/auth.ts");
    const taskIdx = msg.indexOf("HX-Task: S01/T02");
    const resolvesIdx = msg.indexOf("Resolves #43");
    assert.ok(taskIdx > keyFilesIdx, "HX-Task after key files");
    assert.ok(resolvesIdx > taskIdx, "Resolves after HX-Task");
  });
  ...
  it("no Resolves trailer when issueNumber is not set", () => {
    ...
    assert.ok(msg.includes("HX-Task: S01/T02"), "HX-Task trailer still present");
  });
});
