...
/**
 * Check if the worktree has existing HX milestones that would
  ...
  try {
    ...
    if (hasExistingMilestones(info.path)) {
      ...
      const keepExisting = await showConfirm(ctx, {
        ...
        message: [
          `This worktree inherited existing HX milestones from the main branch.`,
          ``,
...
async function handleList(
  ...
  try {
    ...
    if (worktrees.length === 0) {
      ctx.ui.notify("No HX worktrees found. Create one with /worktree <name>.", "info");
      return;
    }
    ...
    const cwd = process.cwd();
    const lines = [CLR.header("HX Worktrees"), ""];
...
async function handleMerge(
  ...
  try {
    ...
    const previewLines = [
      ...
      "",
      `  ${totalChanges} file${totalChanges === 1 ? "" : "s"} changed, ${CLR.ok(`+${totalAdded}`)} ${RED}-${totalRemoved}${RESET} lines ${CLR.muted(`(${codeChanges} code, ${hxChanges} HX)`)}`,
    ];
    ...
    const prompt = loadPrompt("worktree-merge", {
      ...
      hxDiff: hxDiff || "(no HX artifact changes)",
    ...
    ctx.ui.notify(
      `${CLR.ok("✓")} Merge helper started for ${CLR.name(name)} ${CLR.muted(`(${codeChanges} code + ${hxChanges} HX artifact change${totalChanges === 1 ? "" : "s"})`)}`,
      "info",
    );
  } catch (error) {
    const msg = error instanceof Error ? error.message : String(error);
