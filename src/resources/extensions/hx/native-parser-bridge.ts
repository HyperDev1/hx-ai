...
const NATIVE_HX_PARSER_ENABLED = process.env.HX_ENABLE_NATIVE_HX_PARSER === "1";
...
let nativeModule: {
  ...
  batchParseHxFiles: (directory: string) => { files: Array<{ path: string; metadata: string; body: string; sections: string; rawContent: string }>; count: number };
  ...
  scanHxTree: (directory: string) => Array<{ path: string; name: string; isDir: boolean }>;
...
function loadNative(): typeof nativeModule {
  ...
  loadAttempted = true;
  if (!NATIVE_HX_PARSER_ENABLED) return nativeModule;

  try {
    // Dynamic import to avoid hard dependency - fails gracefully if native module not built
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const mod = require('@hyperlab/hx-native');
    if (mod.parseFrontmatter && mod.extractSection && mod.batchParseHxFiles) {
      nativeModule = mod;
    }
  } catch {
...
/**
 * Batch-parse all .md files in a .hx/ directory tree using the native parser.
 * Returns null if native module unavailable.
 */
export function nativeBatchParseHxFiles(directory: string): BatchParsedFile[] | null {
  const native = loadNative();
  if (!native) return null;

  const result = native.batchParseHxFiles(directory);
...
/**
 * Native-backed directory tree scan of a .hx/ directory.
 * Returns a flat list of all entries, or null if native module unavailable.
 */
export function nativeScanHxTree(directory: string): HxTreeEntry[] | null {
  const native = loadNative();
  if (!native) return null;
  return native.scanHxTree(directory);
}
