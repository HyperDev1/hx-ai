/**
 * HX Session Forensics — Deep analysis of pi session JSONL files
 *
 ...
 * - Stuck-retry diagnostics (reading HX activity log copies)
 *
 ...
 * Works with both pi session JSONL and HX activity log JSONL.
 */
 ...
 *
 * Reads the surviving pi session file (or falls back to the last HX activity
...
export function getDeepDiagnostic(basePath: string, worktreePath?: string): string | null {
  ...
  try {
    if (worktreePath) {
      const wtActivityDir = join(hxRoot(worktreePath), "activity");
...
export function readActiveMilestoneId(basePath: string): string | null {
  try {
    const statePath = join(hxRoot(basePath), "STATE.md");
...
function formatTraceSummary(trace: ExecutionTrace): string {
  ...
  // See: https://github.com/hx-build/hx-2/issues/2195
  return parts.join("\n");
}
