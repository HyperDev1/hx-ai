/**
 * HX Command — /hx codebase
 *
 * Generate and manage the codebase map (.hx/CODEBASE.md).
...
const USAGE =
  "Usage: /hx codebase [generate|update|stats]\n\n" +
...
function showStats(basePath: string, ctx: ExtensionCommandContext): void {
  ...
  if (!stats.exists) {
    ctx.ui.notify("No codebase map found. Run /hx codebase generate to create one.", "info");
    return;
  }
  ...
  ctx.ui.notify(
    ...
    (stats.undescribedCount > 0
      ? `Tip: Run /hx codebase update to refresh after file changes.`
...
function resolveCodebaseOptions(args: string, ctx: ExtensionCommandContext): CodebaseMapOptions | false {
  ...
  const prefs = loadEffectiveHXPreferences()?.preferences?.codebase;
