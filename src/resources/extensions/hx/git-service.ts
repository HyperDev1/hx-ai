...
export interface GitPreferences {
  ...
  remote?: string;
  snapshots?: boolean;
  /** Deprecated. .hx/ is managed externally; retained for compatibility. */
  commit_docs?: boolean;
...
/**
 * Thrown when a slice merge hits code conflicts in non-.hx files.
...
export class MergeConflictError extends HXError {
  ...
  constructor(
    ...
    super(
      ...
      `failed with conflicts in ${conflictedFiles.length} non-.hx file(s): ${conflictedFiles.join(", ")}`,
    );
 ...
 * Format: .hx/milestones/<MID>/<MID>-META.json
 */
...
export function writeIntegrationBranch(
  ...
  // .hx/ is managed externally (symlinked) — metadata is not committed to git.
}
...
export class GitServiceImpl {
  ...
  private smartStage(extraExclusions: readonly string[] = []): void {
    ...
    // auto.lock, etc.) — NOT all of .hx/. If .hx/milestones/ files were
    ...
    // discontinuity where the first half of a milestone has .hx/ artifacts
    ...
    if (!this._runtimeFilesCleanedUp) {
      ...
      if (cleaned) {
        nativeCommit(this.basePath, "chore: untrack .hx/ runtime files from git index", { allowEmpty: false });
      }
    ...
    //
    // Exclude only RUNTIME paths from staging — not the entire .hx/ directory.
    // When .hx/milestones/ files are already tracked in the index (projects
    // where .hx/ is not gitignored, or Windows junctions that git sees as
    // real directories), they should continue to be committed. Excluding the
    // entire .hx/ directory mid-milestone causes silent commit failure where
    // the second half of a milestone's artifacts are never committed (#1326).
    //
    // If .hx/ IS in .gitignore (the default for external state projects),
