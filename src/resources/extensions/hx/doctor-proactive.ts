...
import { hxRoot, resolveHxRootFile } from "./paths.js";
...
export async function preDispatchHealthGate(basePath: string): Promise<PreDispatchHealthResult> {
    ...
    if (existsSync(gitDir)) {
      ...
      if (blockers.length > 0) {
        ...
        try {
          ...
          issues.push(`Corrupt git state: ${blockers.join(", ")}. Run /hx doctor fix.`);
        }
      }
    }
  } catch {
  ...
  try {
    ...
    const milestonesDir = join(hxRoot(basePath), "milestones");
    ...
    if (nativeIsRepo(basePath)) {
      ...
      if (state.activeMilestone) {
        ...
        if (resolution.status === "fallback" && resolution.effectiveBranch) {
          ...
          issues.push(
            `${resolution.reason} Restore the branch or update the integration branch before dispatching. Run /hx doctor for details.`,
          );
        }
      }
    }
