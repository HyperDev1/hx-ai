...
export async function autoLoop(
  ...
  while (s.active) {
    ...
    try {
      ...
      //
      // HX_ENGINE_BYPASS=1 skips the engine layer entirely — falls through
      // to the dev path below.
      if (s.activeEngineId != null && s.activeEngineId !== "dev" && !sidecarItem && process.env.HX_ENGINE_BYPASS !== "1") {
