...
import { loadEffectiveHXPreferences, resolveParallelConfig } from "../../preferences.js";
import { projectRoot } from "../context.js";
function emitParallelMessage(pi: ExtensionAPI, content: string): void {
  pi.sendMessage({ customType: "hx-parallel", content, display: true });
}
...
export async function handleParallelCommand(trimmed: string, _ctx: ExtensionCommandContext, pi: ExtensionAPI): Promise<boolean> {
  ...
  emitParallelMessage(pi, `Unknown parallel subcommand "${subcommand}". Usage: /hx parallel [start|status|stop|pause|resume|merge|watch]`);
  return true;
}
