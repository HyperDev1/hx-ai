...
export async function handleSkip(unitArg: string, ctx: ExtensionCommandContext, basePath: string): Promise<void> {
  ...
  const completedKeysFile = pathJoin(basePath, ".hx", "completed-units.json");
  let keys: string[] = [];
  try {
  ...
  keys.push(skipKey);
  mkDir(pathJoin(basePath, ".hx"), { recursive: true });
...
export async function handleDryRun(ctx: ExtensionCommandContext, basePath: string): Promise<void> {
  ...
  const { loadEffectiveHXPreferences: loadPrefs } = await import("./preferences.js");
