...
export function buildLoopRemediationSteps(
  ...
  switch (unitType) {
    case "execute-task": {
      ...
      return [
        `   1. Run \`/hx undo-task ${tid}\` to reset the task state`,
        `   2. Resume auto-mode — it will re-execute the task`,
        `   3. If the task keeps failing, run \`hx recover\` to rebuild DB state from disk`,
      ].join("\n");
    }
    case "plan-slice":
    case "research-slice": {
      ...
      return [
        ...
        `   2. Run \`hx recover\` to rebuild DB state from disk`,
        `   3. Resume auto-mode`,
      ].join("\n");
    }
    case "complete-slice": {
      if (!mid || !sid) break;
      return [
        `   1. Run \`/hx reset-slice ${sid}\` to reset the slice and all its tasks`,
        `   2. Resume auto-mode — it will re-execute incomplete tasks and re-complete the slice`,
        `   3. If the slice keeps failing, run \`hx recover\` to rebuild DB state from disk`,
      ].join("\n");
    }
