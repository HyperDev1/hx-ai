 ...
 * mergeMilestoneToMain auto-resolves .hx/ conflicts by accepting HEAD
...
function createTempRepo(): string {
  ...
  writeFileSync(join(dir, ".hx", "STATE.md"), "version: 1\n");
  run("git add .", dir);
...
test("#2766: stash pop conflict on .hx/ files is auto-resolved", () => {
  ...
  try {
    ...
    // Modify .hx/STATE.md on the milestone branch (diverges from main)
    writeFileSync(join(wtPath, ".hx", "STATE.md"), "version: 2-milestone\n");
    run("git add .", wtPath);
    ...
    // Dirty .hx/STATE.md in the main repo (stash will conflict on pop)
    writeFileSync(join(repo, ".hx", "STATE.md"), "version: 2-main-dirty\n");
    ...
    assert.strictEqual(stashList, "", "stash is empty after .hx/ conflict auto-resolution");
  } finally {
