...
test("Feature 10: collapsible sections JS is included", () => {
  ...
  assert.ok(html.includes("hx-collapsed"), "should reference localStorage key for collapsed state");
});
...
test("Feature 11: dark/light theme toggle JS is included", () => {
  ...
  assert.ok(html.includes("hx-theme"), "should reference localStorage key for theme");
