...
import { openDatabase, closeDatabase, isDbAvailable, insertMilestone, insertSlice, insertTask, updateSliceStatus, upsertMilestonePlanning } from "../hx-db.ts";

// ── Helpers ──────────────────────────────────────────────────────────────────

function createTmpBase(): string {
  return realpathSync(mkdtempSync(join(tmpdir(), "hx-rogue-test-")));
}

/**
 * Create a minimal .hx/ directory structure with a task summary file.
 */
function createTaskSummaryOnDisk(basePath: string, mid: string, sid: string, tid: string): string {
  const tasksDir = join(basePath, ".hx", "milestones", mid, "slices", sid, "tasks");
...
/**
 * Create a minimal .hx/ directory structure with a slice summary file.
 */
function createSliceSummaryOnDisk(basePath: string, mid: string, sid: string): string {
  const sliceDir = join(basePath, ".hx", "milestones", mid, "slices", sid);
...
function createRoadmapOnDisk(basePath: string, mid: string): string {
  const milestoneDir = join(basePath, ".hx", "milestones", mid);
...
test("rogue detection: task summary on disk, no DB row → detected as rogue", () => {
  ...
  const dbPath = join(basePath, ".hx", "hx.db");
  mkdirSync(join(basePath, ".hx"), { recursive: true });
