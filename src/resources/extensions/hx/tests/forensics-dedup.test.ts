...
describe("forensics dedup (#2096)", () => {
  it("forensics_dedup is in KNOWN_PREFERENCE_KEYS", () => {
    const source = readFileSync(join(hxDir, "preferences-types.ts"), "utf-8");
  ...
  it("forensics prompt contains {{dedupSection}} placeholder", () => {
    const prompt = readFileSync(join(hxDir, "prompts", "forensics.md"), "utf-8");
  ...
  it("DEDUP_PROMPT_SECTION contains required search commands", async () => {
    const source = readFileSync(join(hxDir, "forensics.ts"), "utf-8");
    assert.ok(source.includes("DEDUP_PROMPT_SECTION"), "forensics.ts must define DEDUP_PROMPT_SECTION");
    assert.ok(source.includes("gh issue list --repo hx-build/hx-2 --state closed"));
    assert.ok(source.includes("gh pr list --repo hx-build/hx-2 --state open"));
    assert.ok(source.includes("gh pr list --repo hx-build/hx-2 --state merged"));
  });
