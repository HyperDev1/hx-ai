...
describe("forensics dedup (#2096)", () => {
  ...
  it("DEDUP_PROMPT_SECTION contains required search commands", async () => {
    ...
    assert.ok(source.includes("gh issue list --repo gsd-build/hx-2 --state closed"));
    assert.ok(source.includes("gh pr list --repo gsd-build/hx-2 --state open"));
    assert.ok(source.includes("gh pr list --repo gsd-build/hx-2 --state merged"));
  });
