...
const promptsDir = join(process.cwd(), "src/resources/extensions/hx/prompts");
...
// ─── Prompt migration: execute-task → hx_complete_task ───────────────

test("execute-task prompt references hx_complete_task tool", () => {
  const prompt = readPrompt("execute-task");
  assert.match(prompt, /hx_complete_task/);
});
...
test("guided-execute-task prompt references hx_task_complete tool", () => {
  const prompt = readPrompt("guided-execute-task");
  assert.match(prompt, /hx_task_complete/);
});
...
// ─── Prompt migration: complete-slice → hx_complete_slice ────────────

test("complete-slice prompt references hx_complete_slice tool", () => {
  const prompt = readPrompt("complete-slice");
  assert.match(prompt, /hx_complete_slice/);
});
...
test("guided-complete-slice prompt references hx_slice_complete tool", () => {
  const prompt = readPrompt("guided-complete-slice");
  assert.match(prompt, /hx_slice_complete/);
});
...
test("plan-milestone prompt references DB-backed planning tool and explicitly forbids manual roadmap writes", () => {
  ...
  assert.match(prompt, /hx_plan_milestone/);
...
test("plan-slice prompt explicitly names hx_plan_slice as DB-backed planning tool", () => {
  const prompt = readPrompt("plan-slice");
  assert.match(prompt, /hx_plan_slice/);
  assert.match(prompt, /hx_plan_task/);
...
test("plan-slice prompt clarifies hx_plan_slice handles task persistence", () => {
  const prompt = readPrompt("plan-slice");
  // hx_plan_slice persists tasks in its transaction — no separate hx_plan_task calls needed
  assert.match(prompt, /hx_plan_task/);
  assert.match(prompt, /hx_plan_slice` handles task persistence/i);
});

test("replan-slice prompt uses hx_replan_slice as canonical DB-backed tool", () => {
  const prompt = readPrompt("replan-slice");
  assert.match(prompt, /hx_replan_slice/);
...
test("reassess-roadmap prompt references hx_reassess_roadmap tool", () => {
  const prompt = readPrompt("reassess-roadmap");
  assert.match(prompt, /hx_reassess_roadmap/);
});

test("validate-milestone prompt persists verification classes through hx_validate_milestone", () => {
  ...
  assert.match(prompt, /hx_validate_milestone/);
});

// ─── Prompt migration: replan-slice → hx_replan_slice ────────────────

test("replan-slice prompt names hx_replan_slice as the tool to use", () => {
  const prompt = readPrompt("replan-slice");
  assert.match(prompt, /hx_replan_slice/);
});

// ─── Prompt migration: reassess-roadmap → hx_reassess_roadmap ───────

test("reassess-roadmap prompt names hx_reassess_roadmap as the tool to use", () => {
  const prompt = readPrompt("reassess-roadmap");
  assert.match(prompt, /hx_reassess_roadmap/);
});
