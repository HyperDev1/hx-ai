...
function createFixture(): string {
  const b = mkdtempSync(join(tmpdir(), 'hx-edge-'));
  mkdirSync(join(b, '.hx', 'milestones'), { recursive: true });
  return b;
}

function createM(b: string, mid: string, opts?: { roadmap?: boolean; summary?: boolean; dependsOn?: string[] }): void {
  const d = join(b, '.hx', 'milestones', mid);
...
test('EDGE 6: Queue order after discard', async () => {
    ...
    try {
      ...
      const order = JSON.parse(readFileSync(join(b, '.hx', 'QUEUE-ORDER.json'), 'utf-8'));
