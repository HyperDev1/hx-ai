 ...
 *   #1245  Stranded .hx.lock/ directory preventing new sessions
...
describe('session-lock-regression', async () => {
  ...
  {
    const base = mkdtempSync(join(tmpdir(), 'hx-session-lock-'));
    ...
    try {
      ...
      const lockFile = join(hxRoot(base), 'auto.lock');
      assert.ok(!existsSync(lockFile), 'lock file removed after release');

      // The .hx.lock/ directory should be cleaned up
      const lockDir = hxRoot(base) + '.lock';
      assert.ok(!existsSync(lockDir), '.hx.lock/ directory removed after release (#1245)');
    } finally {
  ...
  console.log('\n=== 10. re-entrant acquire refreshes lock artifacts ===');
  {
    ...
    try {
      ...
      if (properLockfileAvailable) {
        assert.ok(existsSync(lockDir), '.hx.lock/ exists after first acquisition');
      }

      const r2 = acquireSessionLock(base);
      assert.ok(r2.acquired, 'second acquisition succeeds');
      if (properLockfileAvailable) {
        assert.ok(existsSync(lockDir), '.hx.lock/ exists after re-entrant acquisition');
      }
      assert.ok(validateSessionLock(base), 'lock remains valid after re-entrant acquisition');

      releaseSessionLock(base);
      assert.ok(!existsSync(lockDir), '.hx.lock/ is removed after release');
    } finally {
      rmSync(base, { recursive: true, force: true });
    }
  }
