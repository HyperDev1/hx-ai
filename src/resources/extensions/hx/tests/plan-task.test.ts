...
import { openDatabase, closeDatabase, insertMilestone, insertSlice, insertTask, getTask } from '../hx-db.ts';
import { handlePlanTask } from '../tools/plan-task.ts';
import { parseTaskPlanFile } from '../files.ts';

function makeTmpBase(): string {
  const base = mkdtempSync(join(tmpdir(), 'hx-plan-task-'));
...
test('handlePlanTask writes planning state and renders task plan', async () => {
  ...
  try {
    ...
    const taskPlanPath = join(base, '.hx', 'milestones', 'M001', 'slices', 'S02', 'tasks', 'T02-PLAN.md');
