...
test("formatProviderReport shows error icon and detail for error status", () => {
  const results: ProviderCheckResult[] = [{
    ...
    detail: "Set ANTHROPIC_API_KEY or run /hx keys",
    required: true,
  }];
  const out = formatProviderReport(results);
...
test("runProviderChecks detects Anthropic key from ANTHROPIC_API_KEY env var", () => {
  ...
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "hx-providers-env-test-")));
...
test("runProviderChecks returns error for Anthropic when no key present", () => {
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "hx-providers-test-")));
  withEnv({
...
test("runProviderChecks optional providers show unconfigured when no key", () => {
  withEnv(
    ...
    () => {
      ...
      process.env.HOME = mkdtempSync(join(tmpdir(), "hx-providers-test-"));
      try {
        const results = runProviderChecks();
...
test("runProviderChecks detects key from auth.json", () => {
  withEnv({ ANTHROPIC_API_KEY: undefined }, () => {
    const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "hx-providers-test-")));
    const agentDir = join(tmpHome, ".hx", "agent");
...
test("runProviderChecks reports ok for Anthropic when GitHub Copilot env var is set", () => {
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "hx-providers-copilot-test-")));
  withEnv({
    ANTHROPIC_API_KEY: undefined,
    ANTHROPIC_OAUTH_TOKEN: undefined,
...
test("runProviderChecks reports ok for Anthropic via GITHUB_TOKEN cross-provider routing", () => {
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "hx-providers-ghtoken-test-")));
  withEnv({
    ANTHROPIC_API_KEY: undefined,
    ANTHROPIC_OAUTH_TOKEN: undefined,
    COPILOT_GITHUB_TOKEN: undefined,
...
test("runProviderChecks detects ANTHROPIC_OAUTH_TOKEN as valid Anthropic auth", () => {
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "hx-providers-oauth-test-")));
  withEnv({
    ANTHROPIC_API_KEY: undefined,
...
test("runProviderChecks reports ok via Copilot auth.json for Anthropic", () => {
  withEnv({
    ...
    const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "hx-providers-copilot-auth-test-")));
    const agentDir = join(tmpHome, ".hx", "agent");
    mkdirSync(agentDir, { recursive: true });
...
test("runProviderChecks uses provider-qualified anthropic-vertex model IDs", () => {
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "hx-providers-vertex-prefix-home-")));
  const repo = realpathSync(mkdtempSync(join(tmpdir(), "hx-providers-vertex-prefix-repo-")));
  mkdirSync(join(repo, ".hx"), { recursive: true });
  writeFileSync(
    join(repo, ".hx", "PREFERENCES.md"),
    [
      "---",
      "models:",
...
test("runProviderChecks uses object provider field for anthropic-vertex models", () => {
  const tmpHome = realpathSync(mkdtempSync(join(tmpdir(), "hx-providers-vertex-provider-home-")));
  const repo = realpathSync(mkdtempSync(join(tmpdir(), "hx-providers-vertex-provider-repo-")));
  mkdirSync(join(repo, ".hx"), { recursive: true });
  writeFileSync(
    join(repo, ".hx", "PREFERENCES.md"),
    [
