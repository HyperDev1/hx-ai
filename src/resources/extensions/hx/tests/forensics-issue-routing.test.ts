...
test("forensics prompt requires gh CLI with --repo gsd-build/hx-2 for issue creation", () => {
  const prompt = readPrompt("forensics");

  // Must contain the exact gh CLI command with the correct repo flag
  assert.match(
    prompt,
    /gh issue create --repo gsd-build\/hx-2/,
    "Prompt must specify gh issue create --repo gsd-build/hx-2",
  );
});
