...
describe('queue-draft-detection', () => {
  test('draft and context milestone detection', async () => {
    const tmpBase = mkdtempSync(join(tmpdir(), "hx-queue-draft-test-"));
    ...
    try {
      ...
      writeFileSync(
        join(hx, "milestones", "M001", "M001-CONTEXT-DRAFT.md"),
        "# M001: Draft Milestone\n\nSeed material from prior discussion.\n",
      );

      // M002: has full CONTEXT.md (ready milestone)
      mkdirSync(join(hx, "milestones", "M002"), { recursive: true });
      writeFileSync(
        join(hx, "milestones", "M002", "M002-CONTEXT.md"),
        "# M002: Ready Milestone\n\nFull context from deep discussion.\n",
      );

      // M003: has both CONTEXT.md and CONTEXT-DRAFT.md (CONTEXT wins)
      mkdirSync(join(hx, "milestones", "M003"), { recursive: true });
      writeFileSync(
        join(hx, "milestones", "M003", "M003-CONTEXT.md"),
        "# M003: Full Context\n\nThis is the real context.\n",
      );
      writeFileSync(
        join(hx, "milestones", "M003", "M003-CONTEXT-DRAFT.md"),
        "# M003: Draft\n\nThis should be ignored.\n",
      );

      // M004: has neither (empty milestone dir)
      mkdirSync(join(hx, "milestones", "M004"), { recursive: true });
