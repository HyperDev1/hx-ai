...
import { openDatabase, closeDatabase, insertMilestone, insertSlice } from "../hx-db.ts";
...
test("dispatch guard blocks when prior milestone has incomplete slices", (t) => {
  ...
  mkdirSync(join(repo, ".hx", "milestones", "M002"), { recursive: true });
  mkdirSync(join(repo, ".hx", "milestones", "M003"), { recursive: true });
  ...
  writeFileSync(join(repo, ".hx", "milestones", "M002", "M002-ROADMAP.md"), "# M002\n");
  writeFileSync(join(repo, ".hx", "milestones", "M003", "M003-ROADMAP.md"), "# M003\n");
...
test("dispatch guard skips completed milestone with SUMMARY even if it has unchecked remediation slices (#1716)", (t) => {
  ...
  writeFileSync(join(repo, ".hx", "milestones", "M001", "M001-ROADMAP.md"), "# M001\n");
  writeFileSync(join(repo, ".hx", "milestones", "M001", "M001-SUMMARY.md"),
    "---\nstatus: complete\n---\n# M001 Summary\nDone.\n");
  writeFileSync(join(repo, ".hx", "milestones", "M002", "M002-ROADMAP.md"), "# M002\n");
...
test("dispatch guard skips cross-milestone check when HX_MILESTONE_LOCK is set (#2797)", (t) => {
  const repo = setupRepo();
  t.after(() => {
  ...
  mkdirSync(join(repo, ".hx", "milestones", "M010"), { recursive: true });
  mkdirSync(join(repo, ".hx", "milestones", "M011"), { recursive: true });
  mkdirSync(join(repo, ".hx", "milestones", "M012"), { recursive: true });
  ...
  writeFileSync(join(repo, ".hx", "milestones", "M010", "M010-ROADMAP.md"), "# M010\n");
  writeFileSync(join(repo, ".hx", "milestones", "M011", "M011-ROADMAP.md"), "# M011\n");
  writeFileSync(join(repo, ".hx", "milestones", "M012", "M012-ROADMAP.md"), "# M012\n");
  ...
  process.env.HX_MILESTONE_LOCK = "M012";
  assert.equal(
    getPriorSliceCompletionBlocker(repo, "main", "execute-task", "M012/S01/T01"),
    null,
  );
