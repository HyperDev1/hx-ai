...
Add the \`completing-milestone\` phase to the HX state machine.
...
- \`agent/extensions/hx/types.ts\` — Phase type definition

## Expected Output

- \`agent/extensions/hx/types.ts\` — Phase union includes \`'completing-milestone'\`
`;
  const result = parseTaskPlanMustHaves(content);
