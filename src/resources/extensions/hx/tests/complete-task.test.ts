...
function tempDbPath(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'hx-complete-task-'));
...
function createTempProject(): { basePath: string; planPath: string } {
  const basePath = fs.mkdtempSync(path.join(os.tmpdir(), 'hx-handler-'));
  const tasksDir = path.join(basePath, '.hx', 'milestones', 'M001', 'slices', 'S01', 'tasks');
  fs.mkdirSync(tasksDir, { recursive: true });

  const planPath = path.join(basePath, '.hx', 'milestones', 'M001', 'slices', 'S01', 'S01-PLAN.md');
...
console.log('\n=== complete-task: accessor stale-state error ===');
{
  // No DB open — accessors should throw HX_STALE_STATE
  closeDatabase();
  let threw = false;
  try {
    ...
    threw = true;
    assertTrue(err.code === 'HX_STALE_STATE' || err.message.includes('No database open'),
      'should throw HX_STALE_STATE when no DB open');
  }
  ...
  try {
    ...
    threw = true;
    assertTrue(err.code === 'HX_STALE_STATE' || err.message.includes('No database open'),
      'insertSlice should throw HX_STALE_STATE');
  }
  ...
  try {
    ...
    threw = true;
    assertTrue(err.code === 'HX_STALE_STATE' || err.message.includes('No database open'),
      'insertTask should throw HX_STALE_STATE');
  }
  ...
  try {
    ...
    threw = true;
    assertTrue(err.code === 'HX_STALE_STATE' || err.message.includes('No database open'),
      'insertVerificationEvidence should throw HX_STALE_STATE');
  }
...
console.log('\n=== complete-task: handler with missing plan file ===');
{
  ...
  const basePath = fs.mkdtempSync(path.join(os.tmpdir(), 'hx-no-plan-'));
  const tasksDir = path.join(basePath, '.hx', 'milestones', 'M001', 'slices', 'S01', 'tasks');
  fs.mkdirSync(tasksDir, { recursive: true });
