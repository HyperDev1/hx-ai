 ...
 *
 * Fix: when the basePath contains `.hx/worktrees/`, resolve the project root
...
function createDir(files: Record<string, string> = {}): string {
  const dir = mkdtempSync(join(tmpdir(), "hx-wt-env-"));
...
describe('doctor-environment-worktree', async () => {
  ...
  try {
    ...
    test('worktree: missing node_modules should NOT error when project root has them', () => {
      ...
      // Simulate a worktree inside .hx/worktrees/<name>/
      const worktreeDir = join(projectRoot, ".hx", "worktrees", "slice-abc");
    ...
    test('worktree: missing node_modules everywhere should still error', () => {
      ...
      const worktreeDir = join(projectRoot, ".hx", "worktrees", "slice-xyz");
      mkdirSync(worktreeDir, { recursive: true });
      writeFileSync(
        join(worktreeDir, "package.json"),
        JSON.stringify({ name: "test-project" }),
    ...
    test('worktree: checkEnvironmentHealth should not add env_dependencies for valid worktree', async () => {
      ...
      const worktreeDir = join(projectRoot, ".hx", "worktrees", "slice-pr");
      mkdirSync(worktreeDir, { recursive: true });
      writeFileSync(
        join(worktreeDir, "package.json"),
        JSON.stringify({ name: "test-project" }),
    ...
    // ── HX_WORKTREE env var detection ─────────────────────────────────
    test('HX_WORKTREE env: should resolve project root node_modules', () => {
      const projectRoot = createDir({
        "package.json": JSON.stringify({ name: "test-project" }),
      });
      mkdirSync(join(projectRoot, "node_modules"), { recursive: true });
      ...
      // Create a directory that doesn't have .hx/worktrees in path but
      // has HX_WORKTREE env pointing to project root
      const someDir = createDir({
        "package.json": JSON.stringify({ name: "test-project" }),
      });
      cleanups.push(someDir);

      const origEnv = process.env.HX_WORKTREE;
      try {
        process.env.HX_WORKTREE = projectRoot;
        const results = runEnvironmentChecks(someDir);
        const depsCheck = results.find(r => r.name === "dependencies");
        assert.ok(
          depsCheck === undefined || depsCheck.status !== "error",
          "HX_WORKTREE env allows fallback to project root node_modules",
        );
      } finally {
        if (origEnv === undefined) {
          delete process.env.HX_WORKTREE;
        } else {
          process.env.HX_WORKTREE = origEnv;
        }
      }
    });
