...
function createTempRepo(): string {
  const dir = realpathSync(
    mkdtempSync(join(tmpdir(), "hx-all-complete-test-")),
  );
  run("git init", dir);
...
function createMilestoneArtifacts(dir: string, mid: string): void {
  const msDir = join(dir, ".hx", "milestones", mid);
...
test("last milestone worktree is merged when it's the final one (#962)", (t) => {
  ...
  const roadmap1 = readFileSync(
    join(tempDir, ".hx", "milestones", "M001", "M001-ROADMAP.md"),
    "utf-8",
  );
  ...
  const roadmap2 = readFileSync(
    join(tempDir, ".hx", "milestones", "M002", "M002-ROADMAP.md"),
    "utf-8",
  );
