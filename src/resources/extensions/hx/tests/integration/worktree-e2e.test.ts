...
describe('worktree-e2e', async () => {
  ...
  try {
    ...
    console.log("\n=== Doctor: orphaned worktree detection ===");
    {
      ...
      const msDir = join(repo, ".hx", "milestones", "M001");
...
`);
      ...
      mkdirSync(join(repo, ".hx", "worktrees"), { recursive: true });
      run("git worktree add -b milestone/M001 .hx/worktrees/M001", repo);

      // Detect
      const detect = await runHXDoctor(repo, { isolationMode: "worktree" });
      ...
      // Fix
      const fixed = await runHXDoctor(repo, { fix: true, isolationMode: "worktree" });
      assert.ok(
