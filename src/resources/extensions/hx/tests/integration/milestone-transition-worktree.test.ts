 ...
 * requires the @hyperlab/hx-coding-agent runtime.
 */
...
function createTempRepo(): string {
  const dir = realpathSync(mkdtempSync(join(tmpdir(), "hx-mt-wt-test-")));
  run("git init", dir);
...
test("worktree swap on milestone transition: merge old, create new", () => {
  ...
  try {
    ...
    const roadmapPath = join(tempDir, ".hx", "milestones", "M001", "M001-ROADMAP.md");
