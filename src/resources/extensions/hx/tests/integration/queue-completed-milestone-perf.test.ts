/**
 * Regression test for #2379: /hx queue fails with 429 rate limit on projects
...
import type { HXState, MilestoneRegistryEntry } from "../../types.ts";
...
const tmpBase = mkdtempSync(join(tmpdir(), "hx-queue-perf-"));
...
for (let i = 1; i <= COMPLETED_COUNT; i++) {
  ...
  mkdirSync(join(hx, "milestones", mid), { recursive: true });
  writeFileSync(
    join(hx, "milestones", mid, `${mid}-CONTEXT.md`),
    `# ${mid}: Completed milestone ${i}\n\nThis is a large context document for ${mid}.\n${"Lorem ipsum dolor sit amet. ".repeat(50)}\n`,
  );
  writeFileSync(
    join(hx, "milestones", mid, `${mid}-SUMMARY.md`),
...
// Create 1 active milestone
{
  ...
  writeFileSync(
    join(hx, "milestones", mid, `${mid}-ROADMAP.md`),
    `# ${mid} Roadmap\n\nSlices planned.\n`,
  );
}
