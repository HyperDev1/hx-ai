...
function createFixtureBase(): string {
  const base = mkdtempSync(join(tmpdir(), 'hx-integration-mixed-'));
...
function createGitRepo(): string {
  const base = mkdtempSync(join(tmpdir(), 'hx-integration-git-'));
  mkdirSync(join(base, '.hx', 'milestones'), { recursive: true });
...
`);
      ...
      mkdirSync(join(base, '.hx', 'milestones', 'M002-abc123'), { recursive: true });
...
test('Group 6: Branch name helpers with new-format IDs', () => {
    ...
    assert.deepStrictEqual(
      ...
      'hx/M001-abc123/S01',
      'G6: getSliceBranchName returns hx/M001-abc123/S01',
    );

    // Test parseSliceBranch with new-format branch name
    const parsed = parseSliceBranch('hx/M001-abc123/S01');
