...
function makeTempRepo(): string {
  const dir = mkdtempSync(join(tmpdir(), "hx-self-heal-"));
...

{
  const cases: Array<{ input: string; shouldContain: string; label: string }> = [
    { input: "CONFLICT (content): Merge conflict in file.ts", shouldContain: "/hx doctor", label: "merge conflict" },
    { input: "error: pathspec 'foo' did not match any file(s)", shouldContain: "/hx doctor", label: "checkout failure" },
    { input: "HEAD detached at abc123", shouldContain: "/hx doctor", label: "detached HEAD" },
    { input: "Unable to create '/path/.git/index.lock': File exists", shouldContain: "/hx doctor", label: "lock file" },
    { input: "fatal: not a git repository", shouldContain: "/hx doctor", label: "not a repo" },
    { input: "some unknown error", shouldContain: "/hx doctor", label: "unknown error" },
  ];

  for (const { input, shouldContain, label } of cases) {
    const result = formatGitError(input);
    assert.ok(result.includes(shouldContain), `${label}: should suggest /hx doctor`);
    console.log(`  ✓ ${label} → suggests /hx doctor`);
  }

  // Test with Error object
  const result = formatGitError(new Error("CONFLICT in merge"));
  assert.ok(result.includes("/hx doctor"), "should handle Error objects");
