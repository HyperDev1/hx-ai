/**
 * Integration test for `hx headless` CLI subcommand
 *
 ...
 *
 * Usage:
 *   npx tsx src/resources/extensions/hx/tests/integration/headless-command.ts
...
Headless proof test project. A minimal fixture used to validate HX auto-mode via RPC.
...
Empty project with HX milestone planned.
...
const FIXTURE_STATE_MD = `# HX State
...
A minimal test project for validating HX auto-mode in headless/RPC mode.
...
- Run HX in headless mode and have it complete a trivial task
...
**Vision:** Prove HX auto-mode works headlessly.

## Success Criteria

- Agent creates hello.txt with content "Hello from headless HX"
...
- hello.txt created with content "Hello from headless HX"

## Verification

- File hello.txt exists in project root with content "Hello from headless HX"
...
- [ ] **T01: Create hello.txt** \`est:5m\`
  ...
  - Files: \`hello.txt\`
  - Do: Create a file called hello.txt in the project root with the content "Hello from headless HX"
  - Verify: File exists with correct content
  - Done when: hello.txt exists with content "Hello from headless HX"
...
Create a file called hello.txt in the project root with the content "Hello from headless HX".

## Steps

1. Create the file hello.txt with the content "Hello from headless HX"
...
- \`hello.txt\` — file containing "Hello from headless HX"
`;

// ── Fixture Creation ─────────────────────────────────────────────────────────

function createFixture(): string {
  const tmpDir = mkdtempSync(join(tmpdir(), "hx-headless-cmd-"));

  // Initialize git repo (HX requires it for branch-per-slice)
  ...
  // Write fixture files
  writeFileSync(join(hxDir, "PROJECT.md"), FIXTURE_PROJECT_MD);
  writeFileSync(join(hxDir, "STATE.md"), FIXTURE_STATE_MD);
  ...
  // Initial commit so HX has a clean git state
...
async function main(): Promise<void> {
  ...
  // Resolve hx-2 repo root (6 levels up from tests/integration/)
  const repoRoot = join(__dirname, "..", "..", "..", "..", "..", "..");

  console.log("=== HX Headless Command Integration Test ===\n");
  ...
  if (artifactExists) {
    ...
    const contentMatch = content === "Hello from headless HX";
