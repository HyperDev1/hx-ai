...
function readContextFile(base: string, mid: string): string {
  return readFileSync(join(base, '.hx', 'milestones', mid, `${mid}-CONTEXT.md`), 'utf-8');
}
...
test('E2E: reorder with depends_on removal', async () => {
  ...
  try {
    ...
    writeFileSync(join(base, '.hx', 'milestones', 'M008', 'M008-CONTEXT.md'), newContent);
...
test('E2E: non-milestone directories filtered from findMilestoneIds (#1494)', () => {
  ...
  try {
    ...
    mkdirSync(join(base, '.hx', 'milestones', 'slices'), { recursive: true });
    mkdirSync(join(base, '.hx', 'milestones', 'temp-backup'), { recursive: true });
...
test('E2E: depends_on inline format preserved after partial removal', () => {
  ...
  try {
    ...
    writeFileSync(join(base, '.hx', 'milestones', 'M008', 'M008-CONTEXT.md'), updated);
...
test('E2E: DB-backed path respects queue order (#2556)', async () => {
    ...
    try {
      ...
      try {
        ...
        // Set queue order: M008 should come FIRST (user reordered via /hx queue)
