...
describe('feature-branch-lifecycle-integration', async () => {
  ...
  try {
    ...
    test('Feature-branch lifecycle with unique milestone IDs', () => {
      ...
      // startAuto bootstraps .hx/ which commits .hx/ files. It also calls
      ...
      mkdirSync(join(repo, ".hx", "milestones", milestoneId), { recursive: true });
      ...
      const worktreeDir = join(repo, ".hx", "worktrees", milestoneId);
    ...
    // ================================================================
    // Test 2: Uncommitted .hx/ planning files are available in worktree
    //
    // When auto-mode starts, .hx/ files may be untracked/uncommitted.
    ...
    test('Untracked planning files copied to worktree', () => {
      ...
      mkdirSync(join(repo, ".hx", "milestones", milestoneId, "slices", "S01", "tasks"), { recursive: true });
      writeFileSync(
        join(repo, ".hx", "milestones", milestoneId, `${milestoneId}-ROADMAP.md`),
        makeRoadmap(milestoneId, "Planning test", [{ id: "S01", title: "First" }]),
      );
      writeFileSync(
        join(repo, ".hx", "milestones", milestoneId, "slices", "S01", "S01-PLAN.md"),
        "# S01: First\n\n**Goal:** Test\n**Demo:** Test\n\n## Tasks\n- [ ] **T01: Do it** `est:10m`\n",
      );
      writeFileSync(join(repo, ".hx", "PROJECT.md"), "# Planning Test Project\n");
      writeFileSync(join(repo, ".hx", "DECISIONS.md"), "# Decisions\n\n## D001\nTest decision.\n");
      ...
      assert.ok(existsSync(wtHx), "worktree .hx exists (symlink or dir)");
    ...
    test('Multiple unique milestones on same feature branch', () => {
      ...
      mkdirSync(join(repo, ".hx", "milestones", mid1), { recursive: true });
      ...
      mkdirSync(join(repo, ".hx", "milestones", mid2), { recursive: true });
