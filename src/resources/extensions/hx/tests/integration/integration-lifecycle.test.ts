...
import { openDatabase, closeDatabase, isDbAvailable, _getAdapter } from '../../hx-db.ts';
...
test('integration-lifecycle: full pipeline', async () => {
    ...
    mkdirSync(join(hxDir, 'milestones', 'M001'), { recursive: true });
    mkdirSync(join(hxDir, 'milestones', 'M002'), { recursive: true });
    ...
    writeFileSync(join(hxDir, 'DECISIONS.md'), decisionsMarkdown);
    writeFileSync(join(hxDir, 'REQUIREMENTS.md'), requirementsMarkdown);
    writeFileSync(join(hxDir, 'milestones', 'M001', 'M001-ROADMAP.md'), ROADMAP_CONTENT);

    const dbPath = join(hxDir, 'test-lifecycle.db');
    ...
    try {
      ...
      const fullDecisionsContent = readFileSync(join(hxDir, 'DECISIONS.md'), 'utf-8');
      const fullRequirementsContent = readFileSync(join(hxDir, 'REQUIREMENTS.md'), 'utf-8');
      ...
      appendFileSync(join(hxDir, 'DECISIONS.md'), '\n' + newDecisionRow + '\n');
      ...
      const regeneratedMd = readFileSync(join(hxDir, 'DECISIONS.md'), 'utf-8');
