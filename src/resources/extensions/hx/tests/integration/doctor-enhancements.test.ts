...
import { runHXDoctor } from "../../doctor.js";
...
describe('doctor-enhancements', async () => {
  ...
  test('circular dependency detection', async () => {
    ...
    const result = await runHXDoctor(base, { fix: false });
    assert.ok(
  ...
  test('dry-run mode', async () => {
    ...
    const result = await runHXDoctor(base, { fix: true, dryRun: true });
  ...
  test('per-check timing', async () => {
    ...
    assert.ok(typeof result.timing?.hxState === "number", "timing.hxState is a number");
  ...
  test('doctor history', async () => {
    ...
    await runHXDoctor(base, { fix: false });
