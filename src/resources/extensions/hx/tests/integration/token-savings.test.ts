...
import { openDatabase, closeDatabase } from '../../hx-db.ts';
...
{
  const base = mkdtempSync(join(tmpdir(), 'hx-token-savings-'));
  mkdirSync(join(base, ".hx"), { recursive: true });
  writeFileSync(join(base, '.hx', 'DECISIONS.md'), decisionsMarkdown);
  writeFileSync(join(base, '.hx', 'REQUIREMENTS.md'), requirementsMarkdown);
  writeFileSync(join(base, '.hx', 'PROJECT.md'), PROJECT_CONTENT);
  ...
  // ── Full-markdown equivalents (what inlineHxRootFile would return) ──
  const fullDecisionsContent = readFileSync(join(base, '.hx', 'DECISIONS.md'), 'utf-8');
  const fullRequirementsContent = readFileSync(join(base, '.hx', 'REQUIREMENTS.md'), 'utf-8');
