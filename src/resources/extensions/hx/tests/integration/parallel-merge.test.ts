...
function createTempRepo(): string {
  ...
  // Mirror production: .hx/worktrees/ is gitignored so autoCommitDirtyState
...
/** Set up a milestone roadmap file in .hx/milestones/<MID>/ */
function setupRoadmap(repo: string, mid: string, title: string, slices: string[]): void {
  const dir = join(repo, ".hx", "milestones", mid);
...
test("mergeCompletedMilestone — clean merge, session status cleaned up", async () => {
  ...
  try {
    ...
    writeSessionStatus(repo, {
      ...
      worktreePath: join(repo, ".hx", "worktrees", "M010"),
    });
    ...
    // Verify commit on main (M010 is now in the body as a HX-Milestone trailer)
    const log = run("git log -1 --format=%B main", repo);
    assert.ok(log.includes("HX-Milestone: M010"), "commit message should reference M010 in trailer");
