 ...
 * circuit breaker.
 *
 * The fix persists the counter to `.hx/runtime/rewrite-count.json`.
 */
...
describe("rewrite-docs circuit breaker persistence (#2203)", () => {
  ...
  test("getRewriteCount handles corrupt JSON gracefully", () => {
    const filePath = join(tempBase, ".hx", "runtime", "rewrite-count.json");
  ...
  test("rewrite-count.json is written to .hx/runtime/", () => {
    setRewriteCount(tempBase, 1);
    const filePath = join(tempBase, ".hx", "runtime", "rewrite-count.json");
