...
test("clearActivityLogState resets dedup state so identical saves write again", () => {
  ...
  // Pre-resolve baseDir so hxRoot() returns a stable key across calls.
  ...
  const baseDir = realpathSync(mkdtempSync(join(tmpdir(), "hx-memleak-test-")));
  try {
    ...
    const actDir = join(baseDir, ".hx", "activity");
...
test("saveActivityLog writes valid JSONL via streaming", () => {
  clearActivityLogState();
  const baseDir = realpathSync(mkdtempSync(join(tmpdir(), "hx-memleak-jsonl-")));
  try {
    const entries = [
