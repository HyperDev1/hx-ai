...
test("init-wizard: existing .hx/ with milestones skips init", (t) => {
  const dir = makeTempDir("existing");
  try {
    mkdirSync(join(dir, ".hx", "milestones", "M001"), { recursive: true });
    mkdirSync(join(dir, ".hx", "milestones", "M002"), { recursive: true });

    const detection = detectProjectState(dir);
    assert.equal(detection.state, "v2-hx");
...
test("init-wizard: empty .hx/ (no milestones) returns v2-hx-empty", (t) => {
  ...
  try {
    ...
    const detection = detectProjectState(dir);
    assert.equal(detection.state, "v2-hx-empty");
    assert.ok(detection.v2);
...
test("init-wizard: v2 .hx/ preferences detected", (t) => {
...
test("init-wizard: CONTEXT.md detected in v2", (t) => {
  ...
  try {
    mkdirSync(join(dir, ".hx", "milestones"), { recursive: true });
    writeFileSync(join(dir, ".hx", "CONTEXT.md"), "# Project Context\n", "utf-8");
...
test("init-wizard: v1 with both .planning/ and .hx/ prioritizes v2", (t) => {
