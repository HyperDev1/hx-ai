/**
 * commands-workflow-custom.test.ts — Tests for `/hx workflow` subcommands
...
import { getHxArgumentCompletions, TOP_LEVEL_SUBCOMMANDS } from "../commands/catalog.ts";
...
function writeDefinition(basePath: string, name: string, content: string): void {
  const defsDir = join(basePath, ".hx", "workflow-defs");
...
describe("workflow catalog registration", () => {
  ...
  it("getHxArgumentCompletions('workflow ') returns six subcommands", () => {
    const completions = getHxArgumentCompletions("workflow ");
  ...
  it("getHxArgumentCompletions('workflow r') filters to run and resume", () => {
    const completions = getHxArgumentCompletions("workflow r");
    const labels = completions.map((c: any) => c.label);
  ...
  it("getHxArgumentCompletions('workflow run ') returns definition names", () => {
    const base = makeTmpBase();
    writeDefinition(base, "deploy-pipeline", SIMPLE_DEF);
    writeDefinition(base, "test-suite", SIMPLE_DEF);

    // Change cwd so the completion scanner can find `.hx/workflow-defs/`
    process.chdir(base);

    const completions = getHxArgumentCompletions("workflow run ");
    const labels = completions.map((c: any) => c.label);
  ...
  it("getHxArgumentCompletions('workflow validate ') returns definition names", () => {
    const base = makeTmpBase();
    writeDefinition(base, "my-workflow", SIMPLE_DEF);

    process.chdir(base);

    const completions = getHxArgumentCompletions("workflow validate ");
    const labels = completions.map((c: any) => c.label);
    assert.ok(labels.includes("my-workflow"), "should include my-workflow");
  });

  it("getHxArgumentCompletions('workflow run d') filters by prefix", () => {
    const base = makeTmpBase();
    writeDefinition(base, "deploy-pipeline", SIMPLE_DEF);
    writeDefinition(base, "test-suite", SIMPLE_DEF);
    ...
    const completions = getHxArgumentCompletions("workflow run d");
    const labels = completions.map((c: any) => c.label);
    assert.ok(labels.includes("deploy-pipeline"), "should include deploy-pipeline");
...
describe("workflow command handler", () => {
  ...
  it("bare '/hx workflow' shows usage", async () => {
    const { handled, notifications } = await callHandler("workflow");
    assert.ok(handled, "should be handled");
    assert.ok(
      notifications.some((n) => n.message.includes("Usage: /hx workflow")),
      "should show usage",
    );
  });

  it("'/hx workflow new' shows skill invocation message", async () => {
  ...
  it("'/hx workflow run' without name shows usage warning", async () => {
  ...
  it("'/hx workflow run nonexistent' shows error for missing definition", async () => {
  ...
  it("'/hx workflow validate' without name shows usage warning", async () => {
  ...
  it("'/hx workflow validate nonexistent' shows definition not found", async () => {
  ...
  it("'/hx workflow pause' without custom engine shows warning", async () => {
  ...
  it("'/hx workflow resume' without custom engine shows warning", async () => {
  ...
  it("'/hx workflow unknown-sub' shows unknown subcommand", async () => {
  ...
  it("'/hx workflow list' with no runs shows empty message", async () => {
