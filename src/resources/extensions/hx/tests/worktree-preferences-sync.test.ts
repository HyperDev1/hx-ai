...
function makeTempDir(prefix: string): string {
  return mkdtempSync(join(tmpdir(), `hx-prefs-test-${prefix}-`));
}
...
test("#2684: syncWorktreeStateBack does NOT overwrite project root PREFERENCES.md", (t) => {
  ...
  mkdirSync(join(wtBase, ".hx", "milestones", mid), { recursive: true });
  mkdirSync(join(mainBase, ".hx", "milestones"), { recursive: true });

  syncWorktreeStateBack(mainBase, wtBase, mid);

  assert.equal(
    readFileSync(join(mainBase, ".hx", "PREFERENCES.md"), "utf-8"),
    rootPrefs,
