...
describe('worktree-sync-milestones', async () => {
  ...
  console.log('\n=== 1. milestone directory synced from main to worktree ===');
  {
    ...
    try {
      const m001Dir = join(mainBase, '.hx', 'milestones', 'M001');
      ...
      assert.ok(!existsSync(join(wtBase, '.hx', 'milestones', 'M001')), 'M001 missing before sync');

      syncProjectRootToWorktree(mainBase, wtBase, 'M001');

      assert.ok(existsSync(join(wtBase, '.hx', 'milestones', 'M001')), '#1311: M001 synced to worktree');
      assert.ok(existsSync(join(wtBase, '.hx', 'milestones', 'M001', 'M001-CONTEXT.md')), 'M001 CONTEXT synced');
      assert.ok(existsSync(join(wtBase, '.hx', 'milestones', 'M001', 'M001-ROADMAP.md')), 'M001 ROADMAP synced');
    } finally {
      cleanup(mainBase);
      cleanup(wtBase);
    }
  ...
  console.log('\n=== 2. missing slices within milestone are synced ===');
  {
    ...
    try {
      ...
      const wtM001Dir = join(wtBase, '.hx', 'milestones', 'M001');
      mkdirSync(join(wtM001Dir, 'slices', 'S01'), { recursive: true });
      writeFileSync(join(wtM001Dir, 'slices', 'S01', 'S01-PLAN.md'), '# S01 Plan');

      syncProjectRootToWorktree(mainBase, wtBase, 'M001');

      assert.ok(existsSync(join(wtBase, '.hx', 'milestones', 'M001', 'slices', 'S02')), '#1311: S02 synced');
      assert.ok(existsSync(join(wtBase, '.hx', 'milestones', 'M001', 'slices', 'S02', 'S02-PLAN.md')), 'S02 PLAN synced');
    } finally {
      cleanup(mainBase);
      cleanup(wtBase);
    }
  ...
  console.log('\n=== 8. syncWorktreeStateBack copies tasks/ subdirectory (#1678) ===');
  {
    ...
    try {
      ...
      const wtSliceDir = join(wtBase, '.hx', 'milestones', 'M001', 'slices', 'S01');
      ...
      const mainSliceDir = join(mainBase, '.hx', 'milestones', 'M001', 'slices', 'S01');
  ...
  console.log('\n=== 9. syncWorktreeStateBack syncs root-level files (REQUIREMENTS, PROJECT) ===');
  {
    ...
    try {
      mkdirSync(join(mainBase, '.hx', 'milestones', 'M001'), { recursive: true });
      mkdirSync(join(wtBase, '.hx', 'milestones', 'M001'), { recursive: true });

      // Main has original REQUIREMENTS and PROJECT
      writeFileSync(join(mainBase, '.hx', 'REQUIREMENTS.md'), '# Requirements\n## R001');
      writeFileSync(join(mainBase, '.hx', 'PROJECT.md'), '# Project\n## Milestone: M001');

      // Worktree has updated versions (complete-milestone added M002 refs)
      writeFileSync(join(wtBase, '.hx', 'REQUIREMENTS.md'), '# Requirements\n## R001\n## R002 — New req');
      writeFileSync(join(wtBase, '.hx', 'PROJECT.md'), '# Project\n## Milestone: M001\n## Milestone: M002');
      writeFileSync(join(wtBase, '.hx', 'KNOWLEDGE.md'), '# Knowledge\nLearned something.');

      const { synced } = syncWorktreeStateBack(mainBase, wtBase, 'M001');

      // Root-level files should be overwritten with worktree versions
      const reqContent = readFileSync(join(mainBase, '.hx', 'REQUIREMENTS.md'), 'utf-8');
      assert.ok(
        reqContent.includes('R002'),
        'REQUIREMENTS.md updated with worktree content',
      );

      const projContent = readFileSync(join(mainBase, '.hx', 'PROJECT.md'), 'utf-8');
      assert.ok(
      ...
      assert.ok(
        existsSync(join(mainBase, '.hx', 'KNOWLEDGE.md')),
  ...
  console.log('\n=== 10. syncWorktreeStateBack syncs all milestone dirs, not just current ===');
  {
    ...
    try {
      ...
      mkdirSync(join(wtBase, '.hx', 'milestones'), { recursive: true });
      ...
      const wtM002Dir = join(wtBase, '.hx', 'milestones', 'M002-abc123');
      ...
      // Main has neither
      assert.ok(
        !existsSync(join(mainBase, '.hx', 'milestones', 'M001')),
        'M001 missing in main before sync',
      );
      assert.ok(
        !existsSync(join(mainBase, '.hx', 'milestones', 'M002-abc123')),
      ...
      assert.ok(
        existsSync(join(mainBase, '.hx', 'milestones', 'M001', 'M001-SUMMARY.md')),
        'M001 SUMMARY synced to main',
      );

      // M002 should ALSO be synced (next milestone — the fix)
      assert.ok(
        existsSync(join(mainBase, '.hx', 'milestones', 'M002-abc123', 'M002-abc123-CONTEXT.md')),
        'M002 CONTEXT synced to main (next-milestone fix)',
      );
      assert.ok(
        existsSync(join(mainBase, '.hx', 'milestones', 'M002-abc123', 'M002-abc123-ROADMAP.md')),
  ...
  console.log('\n=== 11. complete-milestone creates next-milestone artifacts that survive sync ===');
  {
    ...
    try {
      ...
      const mainM006 = join(mainBase, '.hx', 'milestones', 'M006-589wvh');
      ...
      const wtM006 = join(wtBase, '.hx', 'milestones', 'M006-589wvh');
      ...
      const wtM007 = join(wtBase, '.hx', 'milestones', 'M007-wortc8');
      ...
      assert.ok(
        existsSync(join(mainBase, '.hx', 'milestones', 'M006-589wvh', 'M006-589wvh-SUMMARY.md')),
        'M006 SUMMARY synced',
      );
      assert.ok(
        existsSync(join(mainBase, '.hx', 'milestones', 'M006-589wvh', 'slices', 'S01', 'S01-SUMMARY.md')),
        'M006 S01 SUMMARY synced',
      );

      // Verify M007 artifacts synced (the critical fix)
      assert.ok(
        existsSync(join(mainBase, '.hx', 'milestones', 'M007-wortc8', 'M007-wortc8-CONTEXT.md')),
        'M007 CONTEXT synced to main (next-milestone)',
      );
      assert.ok(
        existsSync(join(mainBase, '.hx', 'milestones', 'M007-wortc8', 'M007-wortc8-ROADMAP.md')),
  ...
  console.log('\n=== 12. root files not in worktree are not created in main ===');
  {
    ...
    try {
      ...
      writeFileSync(join(mainBase, '.hx', 'REQUIREMENTS.md'), '# Original');

      const { synced } = syncWorktreeStateBack(mainBase, wtBase, 'M001');

      // Main's REQUIREMENTS should be untouched (worktree had nothing to sync)
      const content = readFileSync(join(mainBase, '.hx', 'REQUIREMENTS.md'), 'utf-8');
      assert.ok(
        content === '# Original',
  ...
  console.log('\n=== 13. QUEUE.md and completed-units.json synced from worktree (#1787) ===');
  {
    ...
    try {
      ...
      writeFileSync(join(wtBase, '.hx', 'QUEUE.md'), '# Queue\n- M002 next');
      writeFileSync(
        join(wtBase, '.hx', 'completed-units.json'),
        JSON.stringify({ units: [{ id: 'M001-S01-T01', completed: true }] }),
      );

      // Main has neither
      assert.ok(
        !existsSync(join(mainBase, '.hx', 'QUEUE.md')),
        'QUEUE.md missing in main before sync',
      );
      assert.ok(
        !existsSync(join(mainBase, '.hx', 'completed-units.json')),
      ...
      const queueContent = readFileSync(join(mainBase, '.hx', 'QUEUE.md'), 'utf-8');
      assert.ok(
      ...
      const cuContent = readFileSync(join(mainBase, '.hx', 'completed-units.json'), 'utf-8');
      assert.ok(
  ...
  console.log('\n=== 14. syncGsdStateToWorktree syncs non-standard milestone dir names (#1547) ===');
  {
    ...
    try {
      ...
      const customDir = join(mainBase, '.hx', 'milestones', 'sprint-alpha');
      mkdirSync(customDir, { recursive: true });
      writeFileSync(join(customDir, 'CONTEXT.md'), '# Sprint Alpha Context');

      const suffixDir = join(mainBase, '.hx', 'milestones', 'M001-abc123');
      ...
      assert.ok(
        existsSync(join(wtBase, '.hx', 'milestones', 'M001-abc123', 'M001-abc123-CONTEXT.md')),
  ...
  console.log('\n=== 15. syncWorktreeStateBack syncs non-standard milestone dir names (#1547) ===');
  {
    ...
    try {
      ...
      const wtCustomDir = join(wtBase, '.hx', 'milestones', 'sprint-beta');
      mkdirSync(wtCustomDir, { recursive: true });
      writeFileSync(join(wtCustomDir, 'SUMMARY.md'), '# Sprint Beta Summary');

      assert.ok(
        !existsSync(join(mainBase, '.hx', 'milestones', 'sprint-beta')),
      ...
      assert.ok(
        existsSync(join(mainBase, '.hx', 'milestones', 'sprint-beta', 'SUMMARY.md')),
