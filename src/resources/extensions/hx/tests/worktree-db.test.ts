...
function tempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'hx-wt-test-'));
}
...
// Test: copies DB file and data is queryable
{
  ...
  const srcDb = path.join(srcDir, 'hx.db');
  const destDb = path.join(destDir, 'nested', 'hx.db');
...
// Test: skips -wal and -shm files
{
  ...
  const destDir = tempDir();
  const srcDb = path.join(srcDir, 'hx.db');
  const destDb = path.join(destDir, 'hx.db');
...
{
  const destDir = tempDir();
  const result = copyWorktreeDb('/nonexistent/path/hx.db', path.join(destDir, 'hx.db'));
...
// Test: creates dest directory if needed
{
  ...
  const destDir = tempDir();
  const srcDb = path.join(srcDir, 'hx.db');
  const deepDest = path.join(destDir, 'a', 'b', 'c', 'hx.db');
...
// Test: merges new decisions from worktree into main
{
  ...
  const wtDir = tempDir();
  const mainDb = path.join(mainDir, 'hx.db');
  const wtDb = path.join(wtDir, 'hx.db');
