...
function createFixtureBase(): string {
  const base = mkdtempSync(join(tmpdir(), 'hx-replan-test-'));
...
function writeTaskSummary(base: string, mid: string, sid: string, tid: string, content: string): void {
  const dir = join(base, '.hx', 'milestones', mid, 'slices', sid, 'tasks');
  mkdirSync(dir, { recursive: true });
...
console.log('\n=== prompt: replan-slice template loads and substitutes variables ===');
{
  ...
  assert.ok(prompt.includes('.hx/milestones/M001/slices/S01/S01-PLAN.md'), 'prompt contains planPath');
...
console.log('\n=== prompt: replan-slice contains preserve-completed-tasks instruction ===');
{
  const prompt = loadPromptFromWorktree('replan-slice', {
    ...
    planPath: '.hx/milestones/M001/slices/S01/S01-PLAN.md',
    blockerTaskId: 'T01',
    replanPath: '.hx/milestones/M001/slices/S01/S01-REPLAN.md',
    inlinedContext: '',
  });
...
console.log('\n=== doctor: blocker + no REPLAN.md → blocker_discovered_no_replan issue ===');
{
  ...
  const report = await runHXDoctor(base, { fix: false, scope: 'M001/S01' });
