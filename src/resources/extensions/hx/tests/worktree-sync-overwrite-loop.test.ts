...
function createBase(name: string): string {
  const base = mkdtempSync(join(tmpdir(), `hx-wt-1886-${name}-`));
...
async function main(): Promise<void> {
  ...
  );
  {
    ...
    try {
      ...
      const prM004 = join(mainBase, ".hx", "milestones", "M004");
      ...
      const wtM004 = join(wtBase, ".hx", "milestones", "M004");
  ...
  console.log("\n=== 2. #1886: missing worktree files still copied ===");
  {
    ...
    try {
      ...
      assertTrue(
        existsSync(join(wtBase, ".hx", "milestones", "M004", "M004-CONTEXT.md")),
        "#1886: missing CONTEXT.md copied from project root",
      );
      assertTrue(
        existsSync(join(wtBase, ".hx", "milestones", "M004", "M004-ROADMAP.md")),
  ...
  );
  {
    ...
    try {
      ...
      writeFileSync(
        join(mainBase, ".hx", "completed-units.json"),
      ...
      const wtCompleted = JSON.parse(
        readFileSync(join(wtBase, ".hx", "completed-units.json"), "utf-8"),
      );
      assertEq(
        wtCompleted,
