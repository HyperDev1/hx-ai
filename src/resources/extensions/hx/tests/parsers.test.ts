...
test('parseRoadmap: full roadmap', () => {
  const content = `# M001: HX Extension — Hierarchical Planning
...
`;
  ...
  assert.deepStrictEqual(r.title, 'M001: HX Extension — Hierarchical Planning', 'roadmap title');
...
Working in the hx-s01 worktree. All imports use .ts extensions.
...
`;
  ...
  assert.ok(c.context.includes('hx-s01 worktree'), 'context content');
