/**
 * Regression test for #1852: removeWorktree targets wrong path when .hx/ is a symlink.
 *
 * When .hx/ is a symlink to an external state directory, git registers
...
// Set up a test repo with .hx/ as a symlink to an external directory,
// mimicking the external state directory layout (~/.hx/projects/<hash>/).
// Resolve tmpdir to handle macOS /tmp -> /private/var/... symlink.
const realTmp = realpathSync(tmpdir());
const base = mkdtempSync(join(realTmp, "hx-wt-symlink-test-"));
const externalState = mkdtempSync(join(realTmp, "hx-wt-symlink-ext-"));
...
assert.ok(existsSync(join(base, ".hx")), ".hx symlink exists");
assert.ok(
  realpathSync(join(base, ".hx")) === externalState,
  ".hx resolves to external state dir",
);
...
describe('worktree-symlink-removal', async () => {
  console.log("\n=== #1852: removeWorktree with symlinked .hx/ ===");
  ...
  unlinkSync(join(base, ".hx"));  // remove the symlink
  mkdirSync(join(base, ".hx", "worktrees", "M002"), { recursive: true });
  // Write a dummy file so the stale directory is non-empty
  writeFileSync(join(base, ".hx", "worktrees", "M002", "stale.txt"), "stale sync artifact", "utf-8");
