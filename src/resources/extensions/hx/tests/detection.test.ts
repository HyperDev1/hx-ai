...
test("detectProjectState: directory with .hx/milestones/M001 returns v2-hx", (t) => {
  const dir = makeTempDir("v2-hx");
  t.after(() => cleanup(dir));

  mkdirSync(join(dir, ".hx", "milestones", "M001"), { recursive: true });
  const result = detectProjectState(dir);
  assert.equal(result.state, "v2-hx");
  assert.ok(result.v2);
  assert.equal(result.v2!.milestoneCount, 1);
});

test("detectProjectState: directory with empty .hx/milestones returns v2-hx-empty", (t) => {
  const dir = makeTempDir("v2-empty");
  t.after(() => cleanup(dir));

  mkdirSync(join(dir, ".hx", "milestones"), { recursive: true });
  const result = detectProjectState(dir);
  assert.equal(result.state, "v2-hx-empty");
  assert.ok(result.v2);
