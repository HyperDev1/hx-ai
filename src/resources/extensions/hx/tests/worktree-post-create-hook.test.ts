...
function makeTmpDir(): string {
  return mkdtempSync(join(tmpdir(), "hx-wt-hook-test-"));
}
...
test("returns error when hook script does not exist", () => {
  ...
  const wt = makeTmpDir();
  try {
    const result = runWorktreePostCreateHook(src, wt, ".hx/hooks/nonexistent");
...
test("executes hook script with correct SOURCE_DIR and WORKTREE_DIR env vars", () => {
  ...
  const wt = makeTmpDir();
  try {
    const hooksDir = join(src, ".hx", "hooks");
    ...
    const result = runWorktreePostCreateHook(src, wt, hookPath(".hx/hooks/post-create"));
...
test("returns error message when hook script fails", () => {
  ...
  try {
    ...
    const result = runWorktreePostCreateHook(src, wt, hookPath(".hx/hooks/failing-hook"));
    assert.ok(result !== null, "should return error string");
