 ...
 * after cancelled /hx sessions.
 *
 * The module-level `reservedMilestoneIds` Set persists across /hx invocations
...
describe("stale milestone ID reservation cleanup (#2488)", () => {
  ...
  test("without cleanup, cancelled sessions inflate the next ID", () => {
    ...
    // Session 1: user starts /hx, ID is previewed and reserved, then cancelled
    ...
    // Session 2: user starts /hx again — stale reservation still in Set
