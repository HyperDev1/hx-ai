// Regression test for #1693 — /hx dispatch uat targets the last completed
...
function createFixture(): string {
  const base = mkdtempSync(join(tmpdir(), "hx-dispatch-uat-"));
...
test("dispatch uat warns when no completed slices exist", async (t) => {
  const base = mkdtempSync(join(tmpdir(), "hx-dispatch-uat-none-"));
  invalidateStateCache();
