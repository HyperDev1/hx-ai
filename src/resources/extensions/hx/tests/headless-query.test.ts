/**
 * Tests for `hx headless query` — single JSON snapshot command.
 *
...
function createFixture(): string {
  const base = mkdtempSync(join(tmpdir(), 'hx-query-test-'))
...
function writeParallelStatus(base: string, mid: string, cost: number): void {
  const dir = join(base, '.hx', 'parallel')
  mkdirSync(dir, { recursive: true })
...
`)
    writeFileSync(
      join(base, '.hx', 'milestones', 'M001', 'M001-SUMMARY.md'),
      '# M001 Summary\n\nComplete.',
    )
