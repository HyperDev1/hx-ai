...
function createBase(): string {
  const base = mkdtempSync(join(tmpdir(), "hx-discuss-queued-"));
...
function writeContext(base: string, mid: string, content: string): void {
  ...
  writeFileSync(join(base, ".hx", "milestones", mid, `${mid}-CONTEXT.md`), content);
}

function writeContextDraft(base: string, mid: string, content: string): void {
  writeMilestoneDir(base, mid);
  writeFileSync(join(base, ".hx", "milestones", mid, `${mid}-CONTEXT-DRAFT.md`), content);
}

function writeRoadmap(base: string, mid: string, content: string): void {
  writeMilestoneDir(base, mid);
  writeFileSync(join(base, ".hx", "milestones", mid, `${mid}-ROADMAP.md`), content);
}
