...
describe('repo-identity-worktree', () => {
  ...
  before(() => {
    base = realpathSync(mkdtempSync(join(tmpdir(), "hx-repo-identity-")));
    stateDir = realpathSync(mkdtempSync(join(tmpdir(), "hx-state-")));
...
test('ensureHxSymlink refreshes repo-meta gitRoot after repo move with fixed project id', () => {
      const moveRepo = realpathSync(mkdtempSync(join(tmpdir(), "hx-repo-identity-move-")));
      ...
      const movedBaseRaw = join(tmpdir(), `hx-repo-identity-moved-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
test('isInheritedRepo detects subdirectory of parent repo without .hx (#1639)', () => {
      const parentRepo = realpathSync(mkdtempSync(join(tmpdir(), "hx-inherited-parent-")));
      ...
      const standaloneRepo = realpathSync(mkdtempSync(join(tmpdir(), "hx-inherited-standalone-")));
...
test('subdirectory of parent repo gets unique identity after git init (#1639)', () => {
      const parentRepo = realpathSync(mkdtempSync(join(tmpdir(), "hx-identity-parent-")));
      run("git init -b main", parentRepo);
      run('git config user.name "Pi Test"', parentRepo);
      run('git config user.email "pi@example.com"', parentRepo);
...
test('ensureHxSymlink from subdirectory does not create .hx in subdir when git-root .hx exists (#2380)', () => {
    const repo = realpathSync(mkdtempSync(join(tmpdir(), "hx-subdir-symlink-")));
