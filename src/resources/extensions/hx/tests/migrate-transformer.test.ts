...
// Tests for transforming parsed PlanningProject into HXProject structures.
// Uses synthetic in-memory fixtures — no filesystem needed.
// Transformer is pure: PlanningProject → HXProject.

import { transformToHX } from '../migrate/transformer.ts';
import type {
  PlanningProject,
  PlanningPhase,
  PlanningPlan,
...
test('Scenario 1: Flat single-milestone', () => {
  ...
  const result = transformToHX(project);
...
test('Scenario 12: Vision derivation', () => {
  ...
  const result1 = transformToHX(project1);
  ...
  const result2 = transformToHX(project2);
