...
test("handleUndo without --force only warns and leaves completed units intact", async () => {
  const base = makeTempDir("hx-undo-confirm");
  try {
    mkdirSync(join(base, ".hx"), { recursive: true });
    mkdirSync(join(base, ".hx", "activity"), { recursive: true });
    writeFileSync(
      join(base, ".hx", "completed-units.json"),
      JSON.stringify(["execute-task/M001/S01/T01"]),
      "utf-8",
    );
    writeFileSync(
      join(base, ".hx", "activity", "001-execute-task-M001-S01-T01.jsonl"),
      "",
      "utf-8",
    );
    ...
    assert.match(notifications[0]?.message ?? "", /Run \/hx undo --force to confirm\./);
    assert.deepEqual(
      JSON.parse(readFileSync(join(base, ".hx", "completed-units.json"), "utf-8")),
...
test("uncheckTaskInPlan flips a checked task back to unchecked", () => {
  const base = makeTempDir("hx-undo-plan");
  try {
...
test("findCommitsForUnit reads the newest matching activity log and dedupes SHAs", () => {
  const base = makeTempDir("hx-undo-activity");
  try {
    const activityDir = join(base, ".hx", "activity");
...
test("handleUndoTask without args shows usage", async () => {
  ...
  const base = makeTempDir("hx-undo-task-usage");
  try {
...
test("handleUndoTask without --force shows confirmation", async () => {
  const base = makeTempDir("hx-undo-task-confirm");
  try {
...
test("handleUndoTask with --force resets task and re-renders plan", async () => {
  const base = makeTempDir("hx-undo-task-force");
  try {
    setupTaskFixture(base);
    const { notifications, ctx } = makeCtx();
    ...
    const summaryPath = join(base, ".hx", "milestones", "M001", "slices", "S01", "tasks", "T01-SUMMARY.md");
    assert.equal(existsSync(summaryPath), false);

    // Plan checkbox unchecked
    const planContent = readFileSync(
      join(base, ".hx", "milestones", "M001", "slices", "S01", "S01-PLAN.md"),
      "utf-8",
    );
...
test("handleUndoTask with non-existent task returns error", async () => {
  const base = makeTempDir("hx-undo-task-notfound");
  try {
    openDatabase(":memory:");
...
test("handleUndoTask accepts partial ID (T01) and resolves from state", async () => {
  const base = makeTempDir("hx-undo-task-partial");
  try {
    setupTaskFixture(base);
...
test("handleResetSlice without args shows usage", async () => {
  const { notifications, ctx } = makeCtx();
  const base = makeTempDir("hx-reset-slice-usage");
  try {
...
test("handleResetSlice without --force shows confirmation", async () => {
  const base = makeTempDir("hx-reset-slice-confirm");
  try {
...
test("handleResetSlice with --force resets slice and all tasks", async () => {
  const base = makeTempDir("hx-reset-slice-force");
  try {
    setupSliceFixture(base);
    const { notifications, ctx } = makeCtx();
...
test("handleResetSlice with non-existent slice returns error", async () => {
  const base = makeTempDir("hx-reset-slice-notfound");
  try {
    openDatabase(":memory:");
    insertMilestone({ id: "M001", title: "Test", status: "active" });
