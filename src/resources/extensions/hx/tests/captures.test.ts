/**
 * Unit tests for HX Captures — file I/O, parsing, and worktree path resolution.
 *
...
test("captures: appendCapture creates CAPTURES.md on first call", (t) => {
  ...
  assert.ok(
    existsSync(join(tmp, ".hx", "CAPTURES.md")),
    "CAPTURES.md should exist",
  );
  const content = readFileSync(join(tmp, ".hx", "CAPTURES.md"), "utf-8");
...
test("captures: resolveCapturesPath returns .hx/CAPTURES.md for normal path", () => {
  const base = join(tmpdir(), "cap-test-project");
  const result = resolveCapturesPath(base);
  assert.ok(result.endsWith(join(".hx", "CAPTURES.md")));
  assert.ok(result.startsWith(base));
});

test("captures: resolveCapturesPath resolves worktree path to project root", () => {
  const base = join(tmpdir(), "cap-test-project");
  const worktreePath = join(base, ".hx", "worktrees", "M004");
  const result = resolveCapturesPath(worktreePath);
  assert.ok(
    result.endsWith(join(".hx", "CAPTURES.md")),
    `should end with .hx/CAPTURES.md, got: ${result}`,
  );
