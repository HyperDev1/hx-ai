...
test("writeLock updates existing lock with new unit info", () => {
  ...
  const data = JSON.parse(readFileSync(join(dir, ".hx", "auto.lock"), "utf-8"));
...
test("clearLock removes the lock file", () => {
  ...
  writeLock(dir, "starting", "M001");
  assert.ok(existsSync(join(dir, ".hx", "auto.lock")), "lock should exist before clear");

  clearLock(dir);
  assert.ok(!existsSync(join(dir, ".hx", "auto.lock")), "lock should be removed after clear");
...
test("bootstrap cleanup releases session lock artifacts", (t) => {
  ...
  assert.ok(existsSync(join(dir, ".hx", "auto.lock")), "auto.lock should exist while lock is held");
  if (properLockfileAvailable) {
    assert.ok(existsSync(join(dir, ".hx.lock")), ".hx.lock should exist while lock is held");
  }

  releaseSessionLock(dir);
  clearLock(dir);

  assert.ok(!existsSync(join(dir, ".hx", "auto.lock")), "auto.lock should be removed by bootstrap cleanup");
  assert.ok(!existsSync(join(dir, ".hx.lock")), ".hx.lock should be removed by bootstrap cleanup");
});
