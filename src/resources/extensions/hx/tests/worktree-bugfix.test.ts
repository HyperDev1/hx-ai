...
describe("worktree-bugfix", () => {
  ...
  it("resolveGitDir returns .git directory in normal repo", () => {
    const repo = mkdtempSync(join(tmpdir(), "hx-wt-fix-"));
    dirs.push(repo);
    initRepo(repo);
  ...
  it("resolveGitDir follows gitdir: pointer in worktree", () => {
    ...
    const wtDir = mkdtempSync(join(tmpdir(), "hx-wt-fix-wt-"));
    dirs.push(wtDir);
  ...
  it("resolveGitDir returns default when .git doesn't exist", () => {
    const noGit = mkdtempSync(join(tmpdir(), "hx-wt-fix-"));
    dirs.push(noGit);
  ...
  it("detectWorktreeName returns name for worktree path", () => {
    assert.deepStrictEqual(
      detectWorktreeName("/project/.hx/worktrees/M005"),
      "M005",
  ...
  it("captureIntegrationBranch is a no-op when in a worktree", () => {
    ...
    const wtPath = join(repo, ".hx", "worktrees", "M005");
    mkdirSync(wtPath, { recursive: true });
    mkdirSync(join(wtPath, ".hx", "milestones", "M005"), { recursive: true });
    // Initialize git in the worktree so getService doesn't fail
    initRepo(wtPath);

    // captureIntegrationBranch should be a no-op — no META.json written
    const metaPath = join(wtPath, ".hx", "milestones", "M005", "M005-META.json");
  ...
  it("detectWorktreeName prevents pull in worktree context", () => {
    ...
    const inWorktree = detectWorktreeName("/project/.hx/worktrees/M006");
