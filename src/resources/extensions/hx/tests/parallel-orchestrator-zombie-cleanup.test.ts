...
function makeTmpBase(): string {
  const base = join(tmpdir(), `hx-test-zombie-${randomUUID()}`);
  mkdirSync(join(base, ".hx", "parallel"), { recursive: true });
  return base;
}
...
function writePersistedState(basePath: string, data: PersistedState): void {
  const dest = join(basePath, ".hx", "orchestrator.json");
  writeFileSync(dest, JSON.stringify(data, null, 2), "utf-8");
}

/** Write a fake session status file to .hx/parallel/<milestoneId>.status.json */
function writeSessionStatusFile(
  ...
  const dest = join(basePath, ".hx", "parallel", `${milestoneId}.status.json`);
  writeFileSync(
    dest,
    JSON.stringify({
      milestoneId,
...
test("#2736: restoreRuntimeState clears stale state when all workers are stopped", (t) => {
  ...
  const stateFile = join(base, ".hx", "orchestrator.json");
