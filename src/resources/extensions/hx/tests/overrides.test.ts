...
function makeTempDir(prefix: string): string {
  const dir = mkdtempSync(join(tmpdir(), `hx-overrides-test-${prefix}-`));
...
describe('overrides', () => {
  ...
  test('appendOverride: creates new file', async () => {
    ...
    const content = readFileSync(join(tmp, ".hx", "OVERRIDES.md"), "utf-8");
    assert.ok(content.includes("# HX Overrides"), "has header");
  ...
  test('loadActiveOverrides: filters to active only', async () => {
    ...
    writeFileSync(join(tmp, ".hx", "OVERRIDES.md"), content, "utf-8");
    const result = await loadActiveOverrides(tmp);
