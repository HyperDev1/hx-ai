 ...
 * prefs to the runner's cwd .hx/PREFERENCES.md and clean up in finally.
 */
...
const RUNNER_PREFS_PATH = join(process.cwd(), ".hx", "PREFERENCES.md");

function writeRunnerPreferences(isolation: "none" | "worktree" | "branch"): void {
  mkdirSync(join(process.cwd(), ".hx"), { recursive: true });
...
test('shouldUseWorktreeIsolation returns false for no prefs (default: none)', () => {
  const globalPrefsExist = existsSync(join(homedir(), ".hx", "PREFERENCES.md"))
    || existsSync(join(homedir(), ".hx", "PREFERENCES.md"));
  if (!globalPrefsExist) {
    try {
