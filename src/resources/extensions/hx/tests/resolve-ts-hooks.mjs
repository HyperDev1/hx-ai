...
export function resolve(specifier, context, nextResolve) {
  ...
  if (specifier.includes('@hx/')) {
    tsSpecifier = specifier.replace('@hx/', PACKAGES_ROOT).replace('/dist/', '/src/');
    if (tsSpecifier.includes('/packages/pi-ai') && !tsSpecifier.endsWith('.ts')) {
        ...
        // Fallback for other hx packages like pi-coding-agent, pi-tui, pi-agent-core
