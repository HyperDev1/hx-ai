...
function createFixtureBase(): string {
  const base = mkdtempSync(join(tmpdir(), 'hx-planning-crossval-'));
...
function scaffoldDirs(base: string, milestoneId: string, sliceIds: string[]): void {
  mkdirSync(join(base, '.hx', 'milestones', milestoneId), { recursive: true });
  for (const sid of sliceIds) {
    mkdirSync(join(base, '.hx', 'milestones', milestoneId, 'slices', sid, 'tasks'), { recursive: true });
  }
}
