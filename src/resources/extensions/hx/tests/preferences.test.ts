...
import type { HXPreferences, HXModelConfigV2, HXPhaseModelConfig } from "../preferences.ts";
...
test("getIsolationMode defaults to none when preferences have no isolation setting", () => {
  ...
  // Default changed from "worktree" to "none" so HX works out of the box
...
test("parses OpenRouter model config with org/model IDs and fallbacks", () => {
  ...
  const prefs = parsePreferencesMarkdown(content);
  assert.notEqual(prefs, null);
  const models = prefs!.models as HXModelConfigV2;
  const research = models.research as HXPhaseModelConfig;
  assert.equal(research.model, "moonshotai/kimi-k2.5");
  assert.deepEqual(research.fallbacks, ["qwen/qwen3.5-397b-a17b"]);
  const execution = models.execution as HXPhaseModelConfig;
...
test("experimental.rtk: non-boolean produces error", () => {
  const result = validatePreferences({ experimental: { rtk: "yes" } } as unknown as HXPreferences);
...
test("experimental: non-object produces error", () => {
  const result = validatePreferences({ experimental: true } as unknown as HXPreferences);
  assert.ok(result.errors.some(e => e.includes("experimental must be an object")));
});

test("experimental: unknown key produces warning", () => {
  const result = validatePreferences({ experimental: { rtk: true, future_flag: true } } as unknown as HXPreferences);
