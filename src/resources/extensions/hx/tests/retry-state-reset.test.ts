// HX Extension — Regression tests for #1714: retry_on signal state reset
//
...
function createRetryFixture(): { base: string; cleanup: () => void } {
  const base = mkdtempSync(join(tmpdir(), "hx-retry-reset-"));
  ...
  const planFile = join(base, ".hx", "milestones", "M001", "slices", "S01", "S01-PLAN.md");
...
test('Retry reset step 2: delete SUMMARY.md', () => {
  const { base, cleanup } = createRetryFixture();
  try {
    const summaryFile = join(base, ".hx", "milestones", "M001", "slices", "S01", "tasks", "T01-SUMMARY.md");
...
test('Retry reset step 3: remove from completedUnits', () => {
  ...
  try {
    ...
    const completedKeysPath = join(base, ".hx", "completed-units.json");
...
test('Retry reset: idempotent when artifacts already missing', () => {
  const base = mkdtempSync(join(tmpdir(), "hx-retry-idempotent-"));
  try {
    ...
    const summaryFile = join(base, ".hx", "milestones", "M001", "slices", "S01", "tasks", `${tid}-SUMMARY.md`);
...
test('resolveHookArtifactPath: correct path for retry artifacts', () => {
  ...
  assert.deepStrictEqual(
    path,
    join(base, ".hx", "milestones", "M001", "slices", "S01", "tasks", "T01-NEEDS-REWORK.md"),
