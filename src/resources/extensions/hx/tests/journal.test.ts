...
function makeTmpBase(): string {
  const base = join(tmpdir(), `hx-journal-test-${randomUUID()}`);
...
describe("emitJournalEvent", () => {
  ...
  test("creates journal directory and JSONL file", () => {
    ...
    const filePath = join(base, ".hx", "journal", "2025-03-21.jsonl");
  ...
  test("silently catches read-only directory errors", () => {
    const journalDir = join(base, ".hx", "journal");
...
describe("emitJournalEvent — auto-creates parent directory", () => {
  let base: string;
  beforeEach(() => {
    base = join(tmpdir(), `hx-journal-test-${randomUUID()}`);
    // Don't create .hx/ — emitJournalEvent should handle it via mkdirSync recursive
  });
  afterEach(() => { cleanup(base); });
