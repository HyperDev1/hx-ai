...
test('prompt-db: re-import updates DB when source markdown changes', () => {
  // Create a temp dir simulating a project with .hx/DECISIONS.md
...
`;
  writeFileSync(join(hxDir, 'DECISIONS.md'), initialDecisions);
...
`;
  writeFileSync(join(hxDir, 'DECISIONS.md'), updatedDecisions);
