 ...
 * written to .hx/metrics.json, and surfaced in the dashboard.
 *
 * Data flow:
...
// in the full barrel (mod.js → ui.js → @hyperlab/hx-tui) which breaks when loaded
...
function metricsPath(base: string): string {
  return join(hxRoot(base), "metrics.json");
}
