...
HX extension source code is at: `{{hxSourceDir}}`
...
5. **Read the actual HX source code** at `{{hxSourceDir}}` to confirm or deny each hypothesis. Do not guess what code does — read it.
...
cat > /tmp/hx-forensic-issue.md << 'HX_ISSUE_BODY'
## Problem
