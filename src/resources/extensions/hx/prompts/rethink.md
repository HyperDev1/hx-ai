...
<!-- NOTE: Park, unpark, reorder, discard, and dependency-update operations are intentionally
     file-based. No hx_* tool API exists for these milestone-lifecycle mutations yet.
     The single-writer DB tools (hx_plan_milestone, hx_complete_milestone, etc.) own
...
### Add a new milestone
Use the `hx_milestone_generate_id` tool to get the next ID, then call `hx_summary_save` with `milestone_id: {ID}`, `artifact_type: "CONTEXT"`, and the scope/goals/success criteria as `content` — the tool writes the context file to disk and persists to DB. Update QUEUE-ORDER.json to place it at the desired position.
