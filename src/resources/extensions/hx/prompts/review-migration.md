## Review Migrated .hx Directory

A `/hx migrate` command just wrote a `.hx/` directory from an old `.planning` source. Your job is to audit the output and verify it meets HX-2 standards before the user starts working with it.
...
- Run `deriveState()` on the `.hx` directory (import from `state.ts`, pass the **project root** as basePath)
...
If the overall result is FAIL, explain what needs manual attention. If PASS WITH NOTES, explain what's imperfect but acceptable. If PASS, confirm the `.hx` directory is ready for HX-2 auto-mode.
