/**
 * HX HTML Report Generator
 *
 * Produces a single self-contained HTML file with:
 *   - Branding header (project name, path, HX version, generated timestamp)
...
<title>HX Report — ${esc(opts.projectName)}${opts.milestoneId ? ` — ${esc(opts.milestoneId)}` : ''}</title>
<style>${CSS}</style>
</head>
<body>
<header>
  <div class="header-inner">
    <div class="branding">
      ...
      <span class="version">v${esc(opts.hxVersion)}</span>
    </div>
...
<footer>
  <div class="footer-inner">
    <span>HX v${esc(opts.hxVersion)}</span>
    <span class="sep">/</span>
...
(function(){
  var saved=JSON.parse(localStorage.getItem('hx-collapsed')||'{}');
  document.querySelectorAll('section[id]').forEach(function(sec){
    ...
    btn.addEventListener('click',function(e){
      ...
      localStorage.setItem('hx-collapsed',JSON.stringify(saved));
    });
  });
...
(function(){
  ...
  btn.textContent=localStorage.getItem('hx-theme')==='light'?'Dark':'Light';
  if(localStorage.getItem('hx-theme')==='light')document.documentElement.classList.add('light-theme');
  btn.addEventListener('click',function(){
    document.documentElement.classList.toggle('light-theme');
    var isLight=document.documentElement.classList.contains('light-theme');
    btn.textContent=isLight?'Dark':'Light';
    localStorage.setItem('hx-theme',isLight?'light':'dark');
  });
  hr.prepend(btn);
})();
`;
