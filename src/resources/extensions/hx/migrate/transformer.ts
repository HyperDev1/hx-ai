// Migration transformer — converts parsed PlanningProject into HXProject.
...
function buildTaskSummary(summary: PlanningSummary): HXTaskSummaryData {
  return {
...
function mapTask(plan: PlanningPlan, index: number, summaries: Record<string, PlanningSummary>): HXTask {
...
function buildSliceSummary(phase: PlanningPhase): HXSliceSummaryData | null {
...
function mapRequirements(reqs: PlanningRequirement[]): HXRequirement[] {
  let autoId = 0;
  return reqs.map((req) => {
    autoId++;
    return {
...
export function transformToHX(parsed: PlanningProject): HXProject {
  const milestones: HXMilestone[] = [];
  ...
  if (isMultiMilestone) {
    // Multi-milestone mode: each roadmap milestone section → one HXMilestone
