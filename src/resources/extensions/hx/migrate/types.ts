...
// ═══════════════════════════════════════════════════════════════════════════
// HX Output Types — produced by transformer, consumed by writer (S03)
// Mirror HX-2 runtime shapes so deriveState() works on migrated output.
// ═══════════════════════════════════════════════════════════════════════════

export interface HXProject {
  milestones: HXMilestone[];
  /** Raw PROJECT.md text (pass through from old format) */
  projectContent: string;
  requirements: HXRequirement[];
  /** Empty or pass-through from old project key decisions */
  decisionsContent: string;
}

export interface HXMilestone {
  /** e.g. "M001", "M002" */
  id: string;
  ...
  boundaryMap: HXBoundaryEntry[];
}

export interface HXSlice {
  /** e.g. "S01", "S02" */
  id: string;
  ...
  summary: HXSliceSummaryData | null;
}
...
export interface HXTask {
  ...
  mustHaves: string[];
  /** Only populated if done */
  summary: HXTaskSummaryData | null;
}

export interface HXRequirement {
  /** e.g. "R001" */
  id: string;
  title: string;
...
export interface HXSliceSummaryData {
...
export interface HXTaskSummaryData {
  completedAt: string;
  provides: string[];
  keyFiles: string[];
  duration: string;
...
export interface HXBoundaryEntry {
  fromSlice: string;
  toSlice: string;
  produces: string;
  consumes: string;
