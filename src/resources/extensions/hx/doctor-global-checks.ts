...
export async function checkGlobalHealth(
  ...
  try {
    ...
    issues.push({
      ...
      scope: "project",
      unitId: "global",
      message: `${orphaned.length} orphaned HX project state director${orphaned.length === 1 ? "y" : "ies"} in ${projectsDir} whose git root no longer exists: ${labels}${overflow}${unknownNote}. Run /hx cleanup projects to audit or /hx cleanup projects --fix to reclaim disk space.`,
      file: projectsDir,
      fixable: true,
    });
