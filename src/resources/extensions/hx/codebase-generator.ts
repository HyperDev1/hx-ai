 ...
 *
 * Produces .hx/CODEBASE.md — a structural table of contents for the project.
 ...
 * Also scans <!-- hx:collapsed-descriptions --> comment blocks to preserve
...
export function parseCodebaseMap(content: string): Map<string, string> {
  ...
  for (const line of content.split("\n")) {
    ...
    if (line.trimStart().startsWith("<!-- hx:collapsed-descriptions")) {
  ...
  for (const group of groups) {
    ...
    if (group.collapsed) {
      ...
      if (descLines.length > 0) {
        lines.push("<!-- hx:collapsed-descriptions");
...
export function updateCodebaseMap(
  ...
  const codebasePath = join(hxRoot(basePath), "CODEBASE.md");
...
/**
 * Write CODEBASE.md to .hx/ directory.
 */
