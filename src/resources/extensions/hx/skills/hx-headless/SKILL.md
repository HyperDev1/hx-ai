---
name: hx-headless
description: Orchestrate HX (Get Shit Done) projects programmatically via headless CLI. Use when an agent needs to create milestones from specs, execute software development workflows, monitor task progress, check project status, or control HX execution (pause/stop/skip/steer). Triggers on requests to "run hx", "create milestone", "execute project", "check hx status", "orchestrate development", "run headless workflow", or any programmatic interaction with the HX project management system. Essential for building orchestrators that coordinate multiple HX workers.
---

# HX Headless Orchestration

Run HX commands without TUI via `hx headless`. Spawns an RPC child process, auto-responds to UI prompts, streams progress.

## Command Syntax

```bash
hx headless [flags] [command] [args...]
```
...
Reads spec, bootstraps `.hx/`, creates milestone, then chains into auto-mode executing all phases (discuss → research → plan → execute → summarize → complete).
...
```bash
hx headless dispatch research|plan|execute|complete|reassess|uat|replan
```
...
PHASE=$(hx headless query | jq -r '.state.phase')
NEXT_ACTION=$(hx headless query | jq -r '.next.action')

case "$PHASE" in
  complete) echo "Done" ;;
  blocked)  echo "Needs intervention" ;;
  *)        [ "$NEXT_ACTION" = "dispatch" ] && hx headless next ;;
esac
```
...
while true; do
  ...
  hx headless query | jq '{phase: .state.phase, progress: .state.progress}'
done
```

### Multi-Session Orchestration

HX tracks concurrent workers via file-based IPC in `.hx/parallel/`. See [references/multi-session.md](references/multi-session.md) for the full architecture.
...
# Monitor all workers: read .hx/parallel/*.status.json
...
**For multiple projects:** each project has its own `.hx/` directory. The orchestrator must track `(projectPath, milestoneId)` tuples externally.
...
```bash
hx headless --json auto 2>/dev/null | while read -r line; do
  TYPE=$(echo "$line" | jq -r '.type')
  case "$TYPE" in
    tool_execution_start) echo "Tool: $(echo "$line" | jq -r '.toolName')" ;;
    extension_ui_request) echo "HX: $(echo "$line" | jq -r '.message // .title // empty')" ;;
...
hx headless --events agent_end,extension_ui_request auto 2>/dev/null

# Only tool execution events
hx headless --events tool_execution_start,tool_execution_end auto
```
...
```bash
hx headless --answers answers.json auto
```
...
## HX Project Structure

All state lives in `.hx/` as markdown files (version-controllable):
