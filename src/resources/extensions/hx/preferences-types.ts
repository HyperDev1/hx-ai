/**
 * Type definitions, constants, and configuration shapes for HX preferences.
 *
...
export interface HXPreferences {
  ...
  /** GitHub sync configuration. Opt-in: syncs HX events to GitHub Issues, Milestones, and PRs. */
  ...
  /**
   * Response language preference. Controls which language HX uses when communicating with the user.
  ...
  /** Slice-level parallel execution. Dispatches eligible slices concurrently via separate worktrees. */
  slice_parallel?: {
    enabled?: boolean;
    max_workers?: number;
  };
  /** Context management tuning for auto-mode. Controls observation masking and tool result truncation. */
  context_management?: {
    observation_masking?: boolean;
    observation_mask_turns?: number;
    tool_result_max_chars?: number;
  };
  /** Codebase map generation settings for /hx codebase. */
  codebase?: {
