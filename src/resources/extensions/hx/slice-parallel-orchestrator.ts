/**
 * HX Slice Parallel Orchestrator — Engine for parallel slice execution
 ...
 * child_process, each running in its own git worktree with HX_SLICE_LOCK
 * + HX_MILESTONE_LOCK env vars set.
 *
 * Key differences from milestone-level parallelism:
 * - Scope: slices within one milestone, not milestones within a project
 * - Lock env: HX_SLICE_LOCK (in addition to HX_MILESTONE_LOCK)
 ...
 *
 * For each eligible slice: create a worktree, spawn `hx --mode json --print "/hx auto"`
 * with env HX_SLICE_LOCK=<SID> + HX_MILESTONE_LOCK=<MID> + HX_PARALLEL_WORKER=1.
 */
 ...
 * The worker runs `hx --mode json --print "/hx auto"` in the slice's worktree
 * with HX_SLICE_LOCK, HX_MILESTONE_LOCK, and HX_PARALLEL_WORKER set.
 */
function spawnSliceWorker(
  ...
  try {
    child = spawn(process.execPath, [binPath, "--mode", "json", "--print", "/hx auto"], {
      cwd: worker.worktreePath,
      env: {
        ...process.env,
        HX_SLICE_LOCK: sliceId,
...
function sliceLogDir(basePath: string): string {
  return join(hxRoot(basePath), "parallel", "slice-logs");
}
