 ...
 * never `git clean` (which would delete untracked .hx/ dirs).
 *
 ...
 * user-friendly messages suggesting `/hx doctor`.
 */
...
const ERROR_PATTERNS: Array<{ pattern: RegExp; message: string }> = [
  {
    ...
    message: "A merge conflict occurred. Code changes on different branches touched the same files. Run `/hx doctor` to diagnose.",
  },
  {
    pattern: /cannot checkout|did not match any|pathspec .* did not match/i,
    message: "Git could not switch branches — the target branch may not exist or the working tree is dirty. Run `/hx doctor` to diagnose.",
  },
  {
    pattern: /HEAD detached|detached HEAD/i,
    message: "Git is in a detached HEAD state — not on any branch. Run `/hx doctor` to diagnose and reattach.",
  },
  {
    pattern: /\.lock|Unable to create .* lock|lock file/i,
    message: "A git lock file is blocking operations. Another git process may be running, or a previous one crashed. Run `/hx doctor` to diagnose.",
  },
  {
    pattern: /fatal: not a git repository/i,
    message: "This directory is not a git repository. Run `/hx doctor` to check your project setup.",
  },
];
 ...
 * a non-technical message suggesting `/hx doctor`. Returns the
...
export function formatGitError(error: string | Error): string {
  ...
  return `A git error occurred: ${errorStr.slice(0, 200)}. Run \`/hx doctor\` for help.`;
}
