  ...
  if (lastMsg && "stopReason" in lastMsg && lastMsg.stopReason === "error") {
    ...
    if (cls.kind === "network") {
      ...
      if (retryState.networkRetryCount < MAX_NETWORK_RETRIES) {
        ...
        setTimeout(() => {
          pi.sendMessage(
            { customType: "hx-auto-timeout-recovery", content: "Continue execution — retrying after transient network error.", display: false },
            { triggerTurn: true },
          );
        }, delayMs);
        return;
          ...
          if (nextModelId) {
            ...
            if (modelToSet) {
              ...
              if (ok) {
                ...
                pi.sendMessage({ customType: "hx-auto-timeout-recovery", content: "Continue execution.", display: false }, { triggerTurn: true });
                return;
              }
            }
          }
