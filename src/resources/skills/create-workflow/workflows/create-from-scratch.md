...
<phase name="write">
Write the file to `.hx/workflow-defs/<name>.yaml`.

Tell the user:
- "Definition saved to `.hx/workflow-defs/<name>.yaml`."
- "Run `/hx workflow validate <name>` to check it against the schema."
- "Run `/hx workflow run <name>` to execute it."
</phase>

<success_criteria>
- A valid YAML file exists at `.hx/workflow-defs/<name>.yaml`
