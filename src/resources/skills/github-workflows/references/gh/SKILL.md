...
```bash
# List open PRs
gh pr list -R hx-build/hx-2

# View PR details
gh pr view <number> -R hx-build/hx-2

# Check PR CI status
gh pr checks <number> -R hx-build/hx-2

# Create PR
gh pr create -R hx-build/hx-2 --title "title" --body "body"

# View PR comments
gh api repos/hx-build/hx-2/pulls/<number>/comments
```
...
```bash
# List issues
gh issue list -R hx-build/hx-2

# List by label
gh issue list -R hx-build/hx-2 --label "priority:p1" --state open
...
gh issue create -R hx-build/hx-2 \
  --title "feat: add feature X" \
  --label "priority:p1" \
  --milestone "v1.0"

# View issue
gh issue view <number> -R hx-build/hx-2

# Close issue with comment
gh issue close <number> -R hx-build/hx-2 --comment "Implemented in PR #N"

# Edit labels on issue
gh issue edit <number> -R hx-build/hx-2 \
...
ISSUE_URL=$(gh issue create -R hx-build/hx-2 \
  --title "..." --body "...")
...
```bash
# List all labels
gh label list -R hx-build/hx-2

# Create label
gh label create "priority:p1" --color "E99695" \
  --description "High priority" -R hx-build/hx-2
```
...
```bash
# List projects
gh project list --owner hx-build

# Create project
gh project create --owner hx-build --title "hx-2 Backlog"

# Add issue to project
gh project item-add 1 --owner hx-build \
  --url https://github.com/hx-build/hx-2/issues/42
```
...
```bash
# List milestones
gh api repos/hx-build/hx-2/milestones
...
gh api repos/hx-build/hx-2/issues/42 \
...
```bash
# List recent runs
gh run list -R hx-build/hx-2 --limit 5

# View specific run
gh run view <run-id> -R hx-build/hx-2

# View failed job logs
gh run view <run-id> -R hx-build/hx-2 --log-failed
```
...
```bash
# List releases
gh release list -R hx-build/hx-2

# View latest release
gh release view --repo hx-build/hx-2
```
...
```bash
# GET request
gh api repos/hx-build/hx-2

# POST with fields
gh api repos/hx-build/hx-2/issues -f title="Bug" -f body="Details"

# GraphQL
gh api graphql -f query='{ viewer { login } }'

# Paginated results
gh api repos/hx-build/hx-2/contributors --paginate
```
...
```bash
# Clone
gh repo clone hx-build/hx-2

# View repo info
gh repo view -R hx-build/hx-2
```
...
```bash
# JSON output
gh pr list -R hx-build/hx-2 --json number,title,state

# JQ filtering
gh pr list -R hx-build/hx-2 --json number,title --jq '.[].title'

# Template formatting
gh pr list -R hx-build/hx-2 --json number,title \
