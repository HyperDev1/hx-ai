...
import { RpcClient, attachJsonlLineReader } from '@hyperlab/hx-coding-agent'
...
export function summarizeToolArgs(toolName: unknown, toolInput: unknown): string {
  ...
  switch (name) {
    ...
    default: {
      // HX tools: show milestone/slice/task IDs when present
      if (name.startsWith('hx_')) {
        return summarizeHxTool(name, input)
      }
...
/** Summarize HX extension tool args into a compact identifier string. */
function summarizeHxTool(name: string, input: Record<string, unknown>): string {
  const parts: string[] = []
  ...
  // Fallback for HX tools without IDs (e.g. hx_decision_save)
  if (input.decision) {
