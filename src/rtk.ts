...
export const HX_RTK_DISABLED_ENV = "HX_RTK_DISABLED";
export const HX_SKIP_RTK_INSTALL_ENV = "HX_SKIP_RTK_INSTALL";
export const HX_RTK_PATH_ENV = "HX_RTK_PATH";
...
export function isRtkEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return !isTruthy(env[HX_RTK_DISABLED_ENV]);
}

function resolveAppRoot(env: NodeJS.ProcessEnv = process.env): string {
  return env.HX_HOME || join(osHomedir(), ".hx");
}
...
export function resolveRtkBinaryPath(options: ResolveRtkBinaryPathOptions = {}): string | null {
  ...
  const explicitPath = env[HX_RTK_PATH_ENV];
...
export async function ensureRtkAvailable(options: EnsureRtkOptions = {}): Promise<EnsureRtkResult> {
  const env = options.env ?? process.env;
  if (!isRtkEnabled(env)) {
    return { enabled: false, supported: true, available: false, source: "disabled", reason: `${HX_RTK_DISABLED_ENV} is set` };
  }
  if (isTruthy(env[HX_SKIP_RTK_INSTALL_ENV])) {
    const configuredPath = env[HX_RTK_PATH_ENV];
    if (configuredPath && existsSync(configuredPath)) {
      return { enabled: true, supported: true, available: true, source: "managed", binaryPath: configuredPath };
    }
    return { enabled: true, supported: true, available: false, source: "missing", reason: `${HX_SKIP_RTK_INSTALL_ENV} is set` };
  }
...
export async function bootstrapRtk(options: EnsureRtkOptions = {}): Promise<EnsureRtkResult> {
  ...
  if (result.binaryPath) {
    process.env[HX_RTK_PATH_ENV] = result.binaryPath;
  }
  return result;
}
