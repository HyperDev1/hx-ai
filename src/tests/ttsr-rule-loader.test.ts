...
function makeTmpProject(): { cwd: string; globalDir: string; projectDir: string; cleanup: () => void } {
	...
	const globalDir = join(cwd, '.hx-global', 'agent', 'rules')
...
// a cwd that has .hx/rules/.

// ═══════════════════════════════════════════════════════════════════════════
// Project-local rule loading
// ═══════════════════════════════════════════════════════════════════════════

test('loads rule from project .hx/rules/', (t) => {
...
test('returns empty array when .hx/rules/ does not exist', (t) => {
	const { cwd, cleanup } = makeTmpProject()
 t.after(() => { cleanup() });

		// cwd exists but no .hx/rules/ dir
		const rules = loadRules(cwd)
