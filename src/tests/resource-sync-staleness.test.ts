 ...
 * with a broken import to persist at ~/.hx/agent/extensions/).
 */
...
test("resource manifest includes contentHash", async (t) => {
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), "hx-resource-test-"));
  ...
  assert.equal(read.hxVersion, "2.28.0");
...
test("missing contentHash in manifest triggers re-sync (upgrade path)", () => {
  ...
  const shouldSkip = oldManifest.hxVersion === "2.28.0"
...
test("matching contentHash skips re-sync", () => {
  ...
  const shouldSkip = manifest.hxVersion === "2.28.0"
