...
import { runVerificationGate } from "../resources/extensions/hx/verification-gate.ts";
...
function withFakeRtk<T>(mapping: Record<string, string | { status?: number; stdout?: string }>, run: () => Promise<T> | T): Promise<T> | T {
  ...
  const previousDisabled = process.env.HX_RTK_DISABLED;
  const previousTimeout = process.env.HX_RTK_REWRITE_TIMEOUT_MS;
  process.env.HX_RTK_PATH = fake.path;
  process.env.HX_RTK_REWRITE_TIMEOUT_MS = "20000";
  ...
  const finalize = () => {
    ...
    if (previousDisabled === undefined) delete process.env.HX_RTK_DISABLED;
    else process.env.HX_RTK_DISABLED = previousDisabled;
    if (previousTimeout === undefined) delete process.env.HX_RTK_REWRITE_TIMEOUT_MS;
    else process.env.HX_RTK_REWRITE_TIMEOUT_MS = previousTimeout;
    fake.cleanup();
  };
...
function withManagedFakeRtk<T>(mapping: Record<string, string | { status?: number; stdout?: string }>, run: (env: NodeJS.ProcessEnv, managedPath: string) => Promise<T> | T): Promise<T> | T {
  ...
  process.env.HX_RTK_REWRITE_TIMEOUT_MS = "20000";
  delete process.env.HX_RTK_PATH;
  delete process.env.HX_RTK_DISABLED;

  const env: NodeJS.ProcessEnv = {
    ...process.env,
    HX_HOME: managedHome,
    HX_RTK_REWRITE_TIMEOUT_MS: "20000",
  };
  delete env.HX_RTK_PATH;

  const finalize = () => {
    if (previousHome === undefined) delete process.env.HX_HOME;
...
test("shared RTK helper falls back to the managed RTK path when HX_RTK_PATH is unset", async () => {
