...
const tempRoot = mkdtempSync(join(tmpdir(), "hx-project-discovery-"));
...
// project-b: empty-hx (.hx folder, no milestones)
const projectB = join(tempRoot, "project-b");
mkdirSync(projectB);
mkdirSync(join(projectB, ".hx"));
...
const monorepoPnpm = mkdtempSync(join(tmpdir(), "hx-mono-pnpm-"));
...
const monorepoLerna = mkdtempSync(join(tmpdir(), "hx-mono-lerna-"));
...
const monorepoWorkspaces = mkdtempSync(join(tmpdir(), "hx-mono-ws-"));
...
const monorepoTurbo = mkdtempSync(join(tmpdir(), "hx-mono-turbo-"));
...
const monorepoNx = mkdtempSync(join(tmpdir(), "hx-mono-nx-"));
...
const plainProject = mkdtempSync(join(tmpdir(), "hx-plain-project-"));
...
describe("project-discovery", () => {
  ...
  test("project-b is detected as empty-hx", () => {
    const results = discoverProjects(tempRoot);
    const b = results.find(r => r.name === "project-b");
    assert.ok(b, "project-b not found");
    assert.equal(b.kind, "empty-hx");
    assert.equal(b.signals.hasHxFolder, true);
  });
...
describe("project-discovery with monorepo root as devRoot", () => {
  ...
  test("monorepo entry has correct kind (brownfield when no .hx)", () => {
    const results = discoverProjects(monorepoPnpm);
