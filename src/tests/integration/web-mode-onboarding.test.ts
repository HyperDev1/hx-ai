...
function makeWorkspaceFixture(): { projectCwd: string; sessionsDir: string; cleanup: () => void } {
  const root = mkdtempSync(join(tmpdir(), "hx-web-onboarding-integration-"));
        ...
        slices: [
          {
            ...
            planPath: ".hx/milestones/M001/slices/S02/S02-PLAN.md",
            tasksDir: ".hx/milestones/M001/slices/S02/tasks",
            tasks: [
              {
                id: "T02",
                title: "Enforce the gate and refresh bridge auth after successful setup",
                done: false,
                planPath: ".hx/milestones/M001/slices/S02/tasks/T02-PLAN.md",
              },
            ],
          },
        ],
...
test("fresh hx --web browser onboarding stays locked on failed validation and unlocks after a successful retry", async (t) => {
  if (process.platform === "win32") {
    t.skip("runtime launch test uses POSIX browser-open stubs")
    return
  }

  const tempRoot = mkdtempSync(join(tmpdir(), "hx-web-onboarding-runtime-"))
