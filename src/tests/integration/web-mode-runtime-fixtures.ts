...
function sessionBaseVariants(baseSessionsDir: string): string[] {
  ...
  if (normalized.endsWith("/.hx/sessions")) {
    variants.add(join(dirname(baseSessionsDir), "agent", "sessions"))
  }
  if (normalized.endsWith("/.hx/agent/sessions")) {
...
export function makeRuntimeWorkspaceFixture(): RuntimeWorkspaceFixture {
  const root = mkdtempSync(join(tmpdir(), "hx-web-runtime-fixture-"))
...
export function makeInterruptedRunRuntimeFixture(): RuntimeWorkspaceFixture {
  const root = mkdtempSync(join(tmpdir(), "hx-web-runtime-recovery-"))
  const projectCwd = join(root, "project")
  const milestoneDir = join(projectCwd, ".hx", "milestones", "M002")
          ...
          content: [
            {
              ...
              id: "tool-read-1",
              name: "read",
              arguments: { path: ".hx/milestones/M002/slices/S04/S04-PLAN.md" },
            },
            {
              type: "toolCall",
              id: "tool-write-1",
