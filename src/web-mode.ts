...
export function stopWebMode(deps: Pick<WebModeDeps, 'pidFilePath' | 'readPidFile' | 'deletePidFile' | 'stderr'> = {}, options: WebModeStopOptions = {}): WebModeStopResult {
  ...
  if (options.all) {
    ...
    for (const [cwd, entry] of entries) {
      ...
      if (result === 'killed') {
        stderr.write(`[hx] Stopped web server for ${cwd} (pid=${entry.pid})\n`)
        stopped++
      } else if (result === 'already-dead') {
        stderr.write(`[hx] Web server for ${cwd} was already stopped (pid=${entry.pid})\n`)
        stopped++
      } else {
        stderr.write(`[hx] Failed to stop web server for ${cwd}: ${result.error}\n`)
      }
    ...
    stderr.write(`[hx] Stopped ${stopped} instance${stopped === 1 ? '' : 's'}.\n`)
  ...
  if (options.projectCwd) {
    ...
    if (!entry) {
      stderr.write(`[hx] No web server running for ${resolvedCwd}\n`)
      return { ok: false, reason: 'not-found' }
    }
    const result = killPid(entry.pid)
    unregisterInstance(resolvedCwd)
    if (result === 'killed') {
      stderr.write(`[hx] Stopped web server for ${resolvedCwd} (pid=${entry.pid})\n`)
      return { ok: true, stoppedCount: 1 }
    } else if (result === 'already-dead') {
      stderr.write(`[hx] Web server for ${resolvedCwd} was already stopped — cleared stale entry.\n`)
      return { ok: true, stoppedCount: 1 }
    } else {
      stderr.write(`[hx] Failed to stop web server for ${resolvedCwd}: ${result.error}\n`)
...
function stopLegacyPidFile(deps: Pick<WebModeDeps, 'pidFilePath' | 'readPidFile' | 'deletePidFile' | 'stderr'>): WebModeStopResult {
  ...
  if (pid === null) {
    stderr.write(`[hx] Web server is not running (no PID file found)\n`)
    return { ok: false, reason: 'no-pid-file' }
  }

  stderr.write(`[hx] Stopping web server (pid=${pid})…\n`)

  const result = killPid(pid)
  deletePid(pidFilePath)
  if (result === 'killed') {
    stderr.write(`[hx] Web server stopped.\n`)
    return { ok: true }
  } else if (result === 'already-dead') {
    stderr.write(`[hx] Web server was already stopped — cleared stale PID file.\n`)
    return { ok: true }
  } else {
    stderr.write(`[hx] Failed to stop web server: ${result.error}\n`)
    return { ok: false, reason: result.error }
  }
}
...
function formatLaunchStatus(status: WebModeLaunchStatus): string {
  if (status.ok) {
    return `[hx] Web mode startup: status=started cwd=${status.cwd} port=${status.port} host=${status.hostPath} kind=${status.hostKind} url=${status.url}\n`
  }

  return `[hx] Web mode startup: status=failed cwd=${status.cwd} port=${status.port ?? 'n/a'} host=${status.hostPath ?? 'unresolved'} kind=${status.hostKind} reason=${status.failureReason}\n`
}
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        if (!hostUp) {
          hostUp = true
          stderr?.write(`[hx] Web host ready.\n`)
        }
        consecutive5xx = 0
    ...
    if (now - lastTickAt >= TICKER_INTERVAL_MS) {
      lastTickAt = now
      if (hostUp) {
        stderr?.write(`[hx] Still waiting… (${elapsed()})\n`)
      } else {
        stderr?.write(`[hx] Waiting for web host… (${elapsed()})\n`)
      }
    }
 ...
 * cleanly.  This handles the "orphan process" scenario where a prior `hx --web`
...
function cleanupStaleInstance(cwd: string, stderr: WritableLike, registryPath?: string): void {
  ...
  stderr.write(`[hx] Cleaning up stale web server for ${key} (pid=${stale.pid}, port=${stale.port})…\n`)
  const result = killPid(stale.pid)
  if (result === 'killed') {
    stderr.write(`[hx] Killed stale web server (pid=${stale.pid}).\n`)
  } else if (result === 'already-dead') {
    stderr.write(`[hx] Stale web server was already stopped (pid=${stale.pid}) — clearing entry.\n`)
  } else {
    stderr.write(`[hx] Could not kill stale web server (pid=${stale.pid}): ${result.error}\n`)
  }
...
export async function launchWebMode(
  ...
  stderr.write(`[hx] Starting web mode…\n`)

  // Kill any stale server instance for this project before reserving a port.
  // This prevents EADDRINUSE when the previous `hx --web` was terminated
  ...
  try {
    stderr.write(`[hx] Initialising resources…\n`)
  ...
  stderr.write(`[hx] Launching web host on port ${port}…\n`)
  ...
  try {
    ...
    try {
      ...
      stderr.write(`[hx] Could not open browser: ${browserError instanceof Error ? browserError.message : String(browserError)}\n`)
    }
  } catch (error) {
    const failure: WebModeLaunchFailure = {
      mode: 'web',
  ...
  stderr.write(`[hx] Ready → ${authenticatedUrl}\n`)
