...
//!
//! Provides high-performance git operations for HX, eliminating the need
...
/// Replaces: `git diff --name-status main...branch -- .hx/`
#[napi]
...
/// `exclude`: optional pathspec to exclude (e.g., ".hx/").
/// Replaces: `git diff main...branch -- .hx/` and `-- . :(exclude).hx/`
#[napi]
...
/// Replaces: `git branch --list milestone/*`, `git branch --list hx/*`
#[napi]
...
/// Simple branch pattern matching for patterns like "milestone/*", "hx/*/*"
fn matches_branch_pattern(name: &str, pattern: &str) -> bool {
    // Handle simple prefix/* patterns
    if let Some(prefix) = pattern.strip_suffix("/*") {
        // For "hx/*/*", this becomes "hx/*" after first strip
        if prefix.contains('*') {
            // Recursive: "hx/*/*" → name must start with "hx/" and have at least 2 segments after
...
/// Replaces: `git branch --merged main --list hx/*`
#[napi]
...
/// Replaces: `git for-each-ref refs/hx/snapshots/ --format=%(refname)`
#[napi]
