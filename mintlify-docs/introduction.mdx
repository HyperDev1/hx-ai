---
title: "HX — Get Shit Done"
description: "An autonomous coding agent that researches, plans, executes, and commits code while you focus on what matters."
---

HX is an autonomous coding agent. Describe what you want built, run `/hx auto`, and walk away. Come back to working software with clean git history.
...
<Tabs>
  <Tab title="Step mode">
    Type `/hx` inside a session. HX executes one unit at a time, pausing between each so you can review.
  ...
  <Tab title="Auto mode">
    Type `/hx auto` and walk away. HX autonomously researches, plans, executes, verifies, and commits until the milestone is complete.
...
/hx capture    # fire-and-forget thoughts
```

## Next steps

<CardGroup cols={2}>
  <Card title="Install HX" icon="download" href="/getting-started">
