...
`Ctrl+Alt+G` or `/hx status` shows real-time cost breakdown by:
...
After two or more slices complete, HX projects the remaining cost:
...
- Check `/hx status` after a few slices to see per-slice averages
