---
title: "Auto mode"
description: "HX's autonomous execution engine — run /hx auto, walk away, come back to built software with clean git history."
---

Auto mode is a **state machine driven by files on disk**. It reads `.hx/STATE.md`, determines the next unit of work, creates a fresh agent session with pre-loaded context, and lets the LLM execute. When the LLM finishes, auto mode reads disk state again and dispatches the next unit.
...
A lock file tracks the current unit. If the session dies, the next `/hx auto` synthesizes a recovery briefing from tool calls that made it to disk and resumes with full context.

**Headless auto-restart:** When running `hx headless auto`, crashes trigger automatic restart with exponential backoff (5s → 10s → 30s cap, default 3 attempts). Combined with crash recovery, this enables overnight "run until done" execution.
...
A sliding-window analysis detects stuck loops — catching cycles like A→B→A→B as well as single-unit repeats. On detection, HX retries once with a diagnostic prompt. If it fails again, auto mode stops with the exact file it expected.
...
HX maintains a `KNOWLEDGE.md` file — an append-only register of project-specific rules, patterns, and lessons learned. The agent reads it at the start of every unit and appends when discovering recurring issues or non-obvious patterns.
...
After milestone completion, HX auto-generates a self-contained HTML report with progress tree, dependency graph, cost/token metrics, execution timeline, and changelog.

```yaml
auto_report: true    # enabled by default
```

Generate manually with `/hx export --html`, or for all milestones with `/hx export --html --all`.

### Reactive task execution

When `reactive_execution: true` is set, HX derives a dependency graph from IO annotations in task plans. Tasks that don't conflict are dispatched in parallel via subagents.
