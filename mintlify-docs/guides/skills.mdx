...
Skills are specialized instruction sets that HX loads when the task matches. They provide domain-specific guidance — coding patterns, framework idioms, testing strategies, and tool usage.

## Bundled skills

HX ships with these skills, installed to `~/.hx/agent/skills/`:
...
The `skill_discovery` preference controls how HX finds skills:
...
1. **Bare name** — e.g., `frontend-design` → scans `~/.hx/agent/skills/` and project skills
2. **Absolute path** — e.g., `/Users/you/.hx/agent/skills/my-skill/SKILL.md`
...
```
.hx/agent/skills/my-project-skill/
  SKILL.md
```
...
```
/hx skill-health              # overview table
/hx skill-health rust-core    # detailed view
/hx skill-health --stale 30   # unused for 30+ days
/hx skill-health --declining  # falling success rates
```
