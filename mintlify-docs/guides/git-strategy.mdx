...
HX uses git for milestone isolation and sequential commits. You choose an **isolation mode** that controls where work happens. The strategy is fully automated — no manual branch management needed.
...
| `worktree` | `.hx/worktrees/<MID>/` | `milestone/<MID>` | Full file isolation |
| `branch` | Project root | `milestone/<MID>` | Submodule-heavy repos |

### `none` mode (default)

Work happens directly on your current branch. No worktree, no milestone branch. HX still commits sequentially with conventional commit messages, but there's no branch isolation. This is the simplest mode and works well for most projects.
...
Conventional commit format with HX metadata in trailers:
...
Adds `.hx/` to `.gitignore` and keeps all planning artifacts local-only. Useful for teams where only some members use HX.

## Worktree management

### Automatic (auto mode)

1. Milestone starts → worktree created at `.hx/worktrees/<MID>/`
