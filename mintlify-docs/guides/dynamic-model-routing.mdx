...
The routing history (`.hx/routing-history.json`) tracks success/failure per tier per unit type. If a tier's failure rate exceeds 20%, future classifications are bumped up.

User feedback (`/hx rate`) is weighted 2x vs automatic outcomes.
