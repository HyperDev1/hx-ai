...
| Command | Description |
|---------|-------------|
| `/hx prefs` | Open the global preferences wizard |
| `/hx prefs global` | Global preferences wizard |
| `/hx prefs project` | Project preferences wizard |
| `/hx prefs status` | Show current files, merged values, and skill status |
...
Tool API keys are stored globally in `~/.hx/agent/auth.json`. Set them once with `/hx config`.
...
HX connects to external MCP servers configured in project files:

- `.mcp.json` — repo-shared config
- `.hx/mcp.json` — local-only config
...
Verify from a HX session: `mcp_servers` → `mcp_discover` → `mcp_call`.
...
When a model fails to switch, HX automatically tries the next model in the `fallbacks` list.
...
|----------|---------|-------------|
| `GSD_HOME` | `~/.hx` | Global HX directory |
