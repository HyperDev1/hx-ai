...
HX's token optimization system has three pillars: **token profiles**, **context compression**, and **complexity-based task routing**.
...
HX can apply deterministic text compression before falling back to section-boundary truncation:
...
HX classifies each task by complexity and routes it to an appropriate model tier.
...
HX tracks success/failure per tier and adjusts classifications over time. User feedback via `/hx rate` is weighted 2x:

```
/hx rate over    # model was overpowered
/hx rate ok      # appropriate
/hx rate under   # too weak
```
