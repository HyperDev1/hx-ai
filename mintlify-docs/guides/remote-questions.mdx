...
Remote questions allow HX to ask for user input via Slack, Discord, or Telegram when running in headless auto-mode. When HX encounters a decision point, it posts the question to your configured channel and polls for a response.
...
5. HX picks up the response and continues
...
| Command | Description |
|---------|-------------|
| `/hx remote` | Show menu and current status |
| `/hx remote slack` | Set up Slack |
| `/hx remote discord` | Set up Discord |
| `/hx remote telegram` | Set up Telegram |
| `/hx remote status` | Show current config and last prompt status |
| `/hx remote disconnect` | Remove configuration |
