...
Check status of a HX project, handle blockers, track costs, and decide next actions.
...
hx headless query | jq '{phase: .state.phase, blockers: .state.blockers, nextAction: .state.nextAction}'

# 2. Option A: Steer around it
hx headless steer "Skip the database dependency, use in-memory storage instead"
...
EOF
hx headless --answers fix.json auto

# 4. Option C: Force a specific phase
hx headless dispatch replan

# 5. Option D: Escalate to user
echo "HX build blocked. Phase: $(hx headless query | jq -r '.state.phase')"
...
hx headless query | jq '.cost.workers'
...
poll_project() {
  STATE=$(hx headless query 2>/dev/null)
  if [ -z "$STATE" ]; then
    echo "NO_PROJECT"
    return
  fi
...
hx headless --resume "$SESSION_ID" --output-format json auto 2>/dev/null
```

## Reading Build Artifacts

After completion, inspect what HX produced:
...
cat .hx/REQUIREMENTS.md

# Milestone summary
cat .hx/milestones/M001-*/M001-*-SUMMARY.md 2>/dev/null

# Git history (HX commits per-slice)
git log --oneline
```
