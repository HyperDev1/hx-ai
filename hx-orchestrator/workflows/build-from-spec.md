...
- `hx` CLI installed (`npm install -g hx-pi`)
...
mkdir -p "$PROJECT_DIR"
cd "$PROJECT_DIR"
git init 2>/dev/null  # HX needs a git repo
```
...
**Fire-and-forget (simplest — HX does everything):**
```bash
cd "$PROJECT_DIR"
RESULT=$(hx headless --output-format json --timeout 0 --context spec.md new-milestone --auto 2>/dev/null)
EXIT=$?
```
...
```bash
RESULT=$(hx headless --bare --output-format json --timeout 0 --context spec.md new-milestone --auto 2>/dev/null)
EXIT=$?
```
...
case $EXIT in
  0)
    ...
    hx headless query | jq '.state.progress'
  ...
  10)
    ...
    hx headless query | jq '{phase: .state.phase, blockers: .state.blockers}'
...
if [ $EXIT -eq 0 ]; then
  ...
  echo "Files created:"
  find . -not -path './.hx/*' -not -path './.git/*' -type f
else
