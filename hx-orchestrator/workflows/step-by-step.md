...
Run HX one unit at a time with decision points between steps. Use this when you need
...
while true; do
  ...
  CURRENT_PHASE=$(hx headless query | jq -r '.state.phase')
  if [ "$CURRENT_PHASE" = "complete" ]; then
    TOTAL_COST=$(hx headless query | jq -r '.cost.total')
  ...
  # Progress report
  PROGRESS=$(hx headless query | jq -r '"\(.state.progress.tasks.done)/\(.state.progress.tasks.total) tasks"')
...
RESULT=$(hx headless --output-format json --context spec.md new-milestone 2>/dev/null)
EXIT=$?
...
while true; do
  ...
  COST=$(hx headless query | jq -r '.cost.total')
...
hx headless query | jq '{phase: .state.phase, task: .state.activeTask}'

# Redirect
hx headless steer "Use SQLite instead of PostgreSQL for storage"

# Continue
hx headless --output-format json next 2>/dev/null
```
...
```bash
hx headless dispatch replan   # Re-plan the current slice
hx headless dispatch execute  # Skip to execution
hx headless dispatch uat      # Jump to user acceptance testing
```
