...
description: >
  Build software products autonomously via HX headless mode. Handles the full
  ...
  "create a project", "run hx", "check build status", or any task that
...
<objective>
You are an autonomous agent that builds software by orchestrating HX as a subprocess.
HX is a headless CLI that plans, codes, tests, and ships software from a spec.
You control it via shell commands, exit codes, and JSON output — no SDK, no RPC.
</objective>

<mental_model>
HX headless is a subprocess you launch and monitor. Think of it like a junior developer
you hand a spec to:

1. You write the spec (what to build)
2. You launch the build (`hx headless ... new-milestone --context spec.md --auto`)
...
You never write application code yourself — HX does that.
</mental_model>

<critical_rules>
- **Flags before command.** `hx headless [--flags] [command] [args]`. Flags after the command are ignored.
...
- **One project directory per build.** Each HX project needs its own directory with a `.hx/` folder.
</critical_rules>
...
Build a ...
EOF
hx headless --output-format json --context spec.md new-milestone --auto 2>/dev/null
```

**Check project state (instant, free):**
```bash
cd /path/to/project
hx headless query | jq '{phase: .state.phase, progress: .state.progress, cost: .cost.total}'
```

**Resume work on an existing project:**
```bash
cd /path/to/project
hx headless --output-format json auto 2>/dev/null
```

**Run one step at a time:**
```bash
RESULT=$(hx headless --output-format json next 2>/dev/null)
...
| `1`  | Error or timeout | Inspect stderr, check `.hx/STATE.md`, retry or escalate |
...
<project_structure>
HX creates and manages all state in `.hx/`:
```
.hx/
...
State is derived from files on disk — checkboxes in ROADMAP.md and PLAN.md are the source of truth for completion. You never need to edit these files. HX manages them. But you can read them to understand progress.
</project_structure>
...
```bash
hx headless --answers answers.json --output-format json auto 2>/dev/null
```
